# Twilio Analytics and Database Optimization Plan

## 1. Objective

This runbook describes how to reduce database load and response time caused by the Twilio Agent Analytics dashboard, Agent Console polling, attendance tracking, CRM phone lookup, Patient Encounter queries, and Chat Conversation writes.
...
7. Do not hold application transactions open while calling Twilio, WhatsApp, OpenAI, or another external service.
...
- access to the Twilio Agent Analytics page;
...
```text
twilio_click_to_call.api.console.get_analytics
twilio_click_to_call.api.console.get_agent_console_data
twilio_click_to_call.api.console.get_reference_context
twilio_click_to_call.api.call.get_call_status
twilio_click_to_call.api.call.get_my_availability
twilio_click_to_call.api.console.heartbeat_agent_console
twilio_click_to_call.api.console.record_agent_activity
```
...
```sql
ALTER TABLE `tabTwilio Agent Attendance Log`
  ADD INDEX `idx_twilio_attendance_user_open_day_modified`
  (`agent_user`, `status`, `shift_date`, `modified`);
```

```sql
ALTER TABLE `tabTwilio Agent Attendance Log`
  ADD INDEX `idx_twilio_attendance_user_tab_open_day_creation`
...
```sql
ALTER TABLE `tabTwilio Agent Attendance Log`
  ADD INDEX `idx_twilio_attendance_user_day_source_online`
...
```text
twilio:attendance:{shift_date}:{user}:{tab_id} -> attendance_record_name
```
...
`twilio_click_to_call.api.reports.get_dashboard_summary` is not referenced by the current frontend. Confirm external consumers through access logs. If none exist:
