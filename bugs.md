...
This document identifies API calls in `twilio_click_to_call` that are unnecessary, duplicated, or more expensive than required. It focuses on calls made repeatedly during normal Desk and Twilio Agent Console usage. User-triggered mutations, provider calls required to start or stop a phone call, and pagination requests are not treated as unnecessary.
...
**Severity:** High  
**Location:** `twilio_click_to_call/twilio_click_to_call/page/twilio_agent_console/twilio_agent_console.js`, lines 530-596
...
```javascript
frappe.call('twilio_click_to_call.api.console.get_agent_console_data', {
    limit: 500,
    // filters omitted
});
```

The backend then rebuilds dynamic data including call capability, active call, and the full lead/patient queue. See `twilio_click_to_call/api/console.py`, lines 250-283.
...
**Location:** `twilio_agent_console.js`, lines 539-564 and 2095-2120
...
- `twilio_agent_console.js`, lines 608-646
- `twilio_click_to_call/public/js/availability.js`, lines 550-598
...
**Severity:** Medium  
**Location:** `twilio_click_to_call/public/js/availability.js`, lines 46-74 and 493-501
...
Disable the global availability refresh timer while the Agent Console owns availability state. Dispatch the existing `twilio_availability_changed` event—or update the shared availability control directly—using availability returned by `get_agent_console_data`.
...
**Severity:** Medium  
**Location:** `twilio_click_to_call/api/console.py`, lines 267-305
...
**Severity:** Medium  
**Location:** `twilio_agent_console.js`, including lines 787, 854, 1384, 1657, 1734, 3125, 3496, and 3684
...
**Location:** `twilio_agent_console.js`, lines 3626-3648
...
- Twilio provider calls to make, retrieve, or hang up a call;
