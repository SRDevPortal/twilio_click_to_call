frappe.pages['twilio-agent-console'].on_page_load = function(wrapper) {
	...
	wrapper.twilio_agent_console = new TwilioAgentConsole(page);
};

frappe.pages['twilio-agent-console'].on_page_show = function(wrapper) {
	if (wrapper.twilio_agent_console) {
		wrapper.twilio_agent_console.on_page_show();
	}
};

frappe.pages['twilio-agent-console'].on_page_hide = function(wrapper) {
	if (wrapper.twilio_agent_console) {
		wrapper.twilio_agent_console.on_page_hide();
	}
};

const TWILIO_WHATSAPP_PAGE_SIZE = 30;
const TWILIO_AGENT_IDLE_MS = 5 * 60 * 1000;
const TWILIO_MISSED_CALL_SEEN_KEY = 'twilio_agent_console_missed_call_seen';

class TwilioAgentConsole {
	constructor(page) {
		this.page = page;
		this.state = {
			queue: [],
	...
	render() {
		this.page.main.html(`
			<div class="twilio-console">
				<div class="twilio-console-head">
					<div>
						<div class="twilio-eyebrow">${__('APP / TWILIO CALL CENTER / DIALER')}</div>
						<h2>${__('Twilio Agent Call Center')}</h2>
					</div>
					<div class="twilio-head-actions">
						<div class="twilio-head-active-call hidden" data-role="head-active-call">
							<span class="twilio-head-call-pulse"></span>
							<div class="twilio-head-call-copy">
						...
						<div class="twilio-agent-state">
							<span class="twilio-state-dot"></span>
							<span data-role="availability">${__('Checking')}</span>
					</div>
					</div>
				</div>

				<section class="twilio-band twilio-dialer-control">
					<div>
				...
				<div class="twilio-layout">
					<section class="twilio-band twilio-queue">
						<div class="twilio-section-title">
							<h3 data-role="queue-title">${__('Lead Queue')}</h3>
							<div class="twilio-queue-tools">
								...
								<button class="btn btn-default btn-sm twilio-filter-btn" data-action="open-filters">
							...
							<table class="table table-sm twilio-table">
								<thead>
									<tr>
										...
										<th class="twilio-patient-col hidden">${__('Department')}</th>
										<th class="twilio-patient-col hidden">${__('Disease')}</th>
										<th class="twilio-patient-col hidden">${__('Language')}</th>
										<th class="twilio-patient-col hidden">${__('Follow-up ID')}</th>
										<th class="twilio-patient-col hidden">${__('Day')}</th>
										<th class="twilio-team-col">${__('Team')}</th>
										<th class="twilio-lead-owner-col">${__('Lead Owner')}</th>
						...
						<div class="twilio-pagination" data-role="queue-pagination">
							<div class="twilio-page-summary" data-role="queue-page-summary"></div>
							<div class="twilio-page-controls">
								...
								<span class="twilio-page-number" data-role="queue-page-number"></span>
	...
	inject_styles() {
		if ($('#twilio-agent-console-style').length) return;
		$('head').append(`
			<style id="twilio-agent-console-style">
				.twilio-console { background: #f7f7fb; margin: 0 -15px -15px; min-height: calc(100vh - 72px); overflow-x: hidden; padding: 24px; }
				.twilio-console-head { align-items: center; display: flex; justify-content: space-between; margin-bottom: 20px; }
				.twilio-console-head h2 { font-size: 22px; font-weight: 700; margin: 0; }
				.twilio-eyebrow { color: #6b7280; font-size: 11px; font-weight: 700; letter-spacing: .04em; margin-bottom: 4px; }
				.twilio-head-actions { align-items: center; display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-end; }
				.twilio-head-active-call { align-items: center; background: #fff7ed; border: 1px solid #fdba74; border-radius: 7px; display: flex; gap: 9px; max-width: 420px; padding: 5px 7px 5px 10px; }
				.twilio-head-active-call.hidden { display: none; }
				.twilio-head-call-pulse { animation: twilio-call-pulse 1.4s infinite; background: #16a34a; border-radius: 50%; flex: 0 0 auto; height: 9px; width: 9px; }
				.twilio-head-call-copy { display: flex; flex-direction: column; line-height: 1.15; min-width: 0; }
				.twilio-head-call-copy small { color: #9a3412; font-size: 10px; font-weight: 800; text-transform: uppercase; }
				.twilio-head-call-copy strong { color: #431407; max-width: 230px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
				@keyframes twilio-call-pulse { 0%, 100% { box-shadow: 0 0 0 0 rgba(22, 163, 74, .35); } 50% { box-shadow: 0 0 0 5px rgba(22, 163, 74, 0); } }
				.twilio-agent-state { align-items: center; background: #fff; border: 1px solid #e5e7eb; border-radius: 6px; display: flex; gap: 8px; padding: 8px 12px; }
				.twilio-state-dot { background: #16a34a; border-radius: 50%; height: 9px; width: 9px; }
				.twilio-stats { display: grid; gap: 16px; grid-template-columns: repeat(6, minmax(0, 1fr)); margin-bottom: 16px; }
				.twilio-stat { background: #fff; border: 1px solid #ebeef2; border-radius: 8px; padding: 18px; }
				.twilio-stat.clickable { cursor: pointer; transition: border-color .15s ease, transform .15s ease; }
				.twilio-stat.clickable:hover { border-color: #b8d8ff; transform: translateY(-1px); }
				.twilio-stat strong { display: block; font-size: 28px; line-height: 1.1; margin-top: 10px; }
				.twilio-stat span { color: #4b5563; font-size: 12px; font-weight: 700; }
				.twilio-stat small { color: #6b7280; display: block; font-size: 11px; margin-top: 7px; min-height: 15px; }
				.twilio-stat.danger { border-top: 4px solid #dc2626; }
				.twilio-stat.success { border-top: 4px solid #16a34a; }
				.twilio-stat.warning { border-top: 4px solid #ca8a04; }
				.twilio-stat.info { border-top: 4px solid #0284c7; }
				.twilio-stat.neutral { border-top: 4px solid #64748b; }
				.twilio-performance-head { align-items: center; display: flex; justify-content: space-between; margin-bottom: 14px; }
				.twilio-performance-head h4 { font-size: 18px; font-weight: 800; margin: 2px 0 0; }
				.twilio-perf-kpis { display: grid; gap: 12px; grid-template-columns: repeat(4, minmax(0, 1fr)); margin-bottom: 18px; }
				.twilio-perf-kpi { background: #f9fafb; border: 1px solid #eef0f3; border-radius: 8px; padding: 12px; }
				.twilio-perf-kpi span { color: #6b7280; display: block; font-size: 11px; font-weight: 800; margin-bottom: 6px; text-transform: uppercase; }
				.twilio-perf-kpi strong { font-size: 24px; line-height: 1; }
				.twilio-performance-section { border-top: 1px solid #eef0f3; margin-top: 14px; padding-top: 14px; }
				.twilio-performance-section h4 { font-size: 15px; font-weight: 800; margin: 0 0 10px; }
				.twilio-performance-table { table-layout: auto; }
				.twilio-performance-table th { color: #6b7280; font-size: 11px; font-weight: 800; }
				.twilio-performance-table td { vertical-align: middle; white-space: normal; word-break: break-word; }
				.twilio-performance-table code { white-space: nowrap; }
				.twilio-icon { align-items: center; border: 1px solid currentColor; border-radius: 6px; display: inline-flex; height: 30px; justify-content: center; width: 30px; }
				.twilio-band { background: #fff; border: 1px solid #ebeef2; border-radius: 8px; margin-bottom: 16px; padding: 16px; }
				.twilio-dialer-control { align-items: center; display: flex; justify-content: space-between; }
				.twilio-actions { display: flex; gap: 10px; }
					.twilio-layout { display: grid; gap: 16px; grid-template-columns: minmax(0, 1fr); }
				.twilio-section-title { align-items: center; display: flex; gap: 12px; justify-content: space-between; margin-bottom: 12px; }
				.twilio-section-title h3 { font-size: 15px; font-weight: 700; margin: 0; }
				.twilio-queue-tools { display: flex; gap: 8px; justify-content: flex-end; min-width: 280px; }
				.twilio-filter-btn { align-items: center !important; display: inline-flex !important; flex: 0 0 auto; gap: 6px; justify-content: center; min-width: 86px; white-space: nowrap !important; width: auto !important; }
				.twilio-filter-btn .filter-icon, .twilio-filter-btn .button-label { display: inline-block; line-height: 1; white-space: nowrap; }
				.twilio-queue-tools select { max-width: 150px; }
				.twilio-queue-tools input { max-width: 260px; }
				.twilio-table-wrap { overflow-x: auto; }
				.twilio-table { margin: 0; table-layout: fixed; }
				.twilio-table th { color: #6b7280; font-size: 11px; font-weight: 700; }
				.twilio-table td { overflow: hidden; text-overflow: ellipsis; vertical-align: middle; white-space: nowrap; }
				.twilio-table .hidden { display: none; }
				.twilio-pagination { align-items: center; border-top: 1px solid #eef0f3; display: flex; gap: 12px; justify-content: space-between; margin-top: 12px; padding-top: 12px; }
				.twilio-page-summary { color: #6b7280; font-size: 12px; }
				.twilio-page-controls { align-items: center; display: flex; gap: 8px; }
				.twilio-page-controls select { width: 78px; }
				.twilio-page-number { color: #374151; font-size: 12px; font-weight: 700; min-width: 86px; text-align: center; }
				.twilio-person { align-items: center; display: flex; gap: 9px; min-width: 0; }
				.twilio-avatar { align-items: center; background: #eaf3ff; border-radius: 50%; color: #2563eb; display: inline-flex; flex: 0 0 auto; font-weight: 700; height: 28px; justify-content: center; width: 28px; }
				.twilio-status { font-size: 12px; font-weight: 700; }
				.twilio-status.New { color: #0284c7; } .twilio-status.Qualified, .twilio-status.Converted { color: #16a34a; }
				.twilio-status.Not { color: #dc2626; } .twilio-status.Contacted { color: #ca8a04; }
				.twilio-missed-cell { align-items: center; display: inline-flex; height: 28px; justify-content: center; min-width: 32px; position: relative; }
				.twilio-missed-cell.clickable { cursor: pointer; }
				.twilio-missed-count { align-items: center; background: #f3f4f6; border: 1px solid #eef0f3; border-radius: 6px; color: #111827; display: inline-flex; font-size: 12px; font-weight: 900; height: 28px; justify-content: center; min-width: 32px; padding: 0 7px; }
				button.twilio-missed-cell { background: transparent; border: 0; padding: 0; }
				button.twilio-missed-cell:focus .twilio-missed-count, button.twilio-missed-cell:hover .twilio-missed-count { background: #fee2e2; border-color: #fecaca; color: #991b1b; }
				.twilio-missed-badge { align-items: center; background: #ef4444; border: 2px solid #fff; border-radius: 999px; color: #fff; display: inline-flex; font-size: 9px; height: 18px; justify-content: center; line-height: 1; min-width: 18px; position: absolute; right: -6px; top: -7px; }
				.twilio-missed-badge .fa { transform: rotate(135deg); }
				.twilio-missed-cell.has-new .twilio-missed-badge { animation: twilio-missed-pulse 1.2s ease-in-out infinite; box-shadow: 0 0 0 0 rgba(239, 68, 68, 0.45); }
				.twilio-missed-empty { opacity: 0.75; }
				@keyframes twilio-missed-pulse { 0% { box-shadow: 0 0 0 0 rgba(220, 38, 38, 0.45); } 70% { box-shadow: 0 0 0 7px rgba(220, 38, 38, 0); } 100% { box-shadow: 0 0 0 0 rgba(220, 38, 38, 0); } }
				.twilio-missed-list { display: grid; gap: 10px; max-height: 520px; overflow: auto; }
				.twilio-missed-row { border: 1px solid #eef0f3; border-radius: 8px; display: grid; gap: 8px; padding: 10px; }
				.twilio-missed-row-head { align-items: center; display: flex; gap: 10px; justify-content: space-between; }
				.twilio-missed-row-head strong { font-size: 13px; }
				.twilio-missed-row-grid { display: grid; gap: 8px; grid-template-columns: repeat(3, minmax(0, 1fr)); }
				.twilio-missed-row-grid span { color: #6b7280; display: block; font-size: 11px; font-weight: 800; margin-bottom: 2px; }
				.twilio-missed-row-grid div { min-width: 0; overflow-wrap: anywhere; }
				.twilio-wa-queue { align-items: center; border-radius: 999px; display: inline-flex; font-size: 12px; font-weight: 800; gap: 5px; justify-content: center; min-height: 26px; min-width: 52px; padding: 3px 9px; }
				.twilio-wa-queue.has-new { background: #dcfce7; border-color: #86efac; color: #15803d; }
				.twilio-wa-queue.is-quiet { color: #dc2626; }
				.twilio-wa-count { background: #16a34a; border-radius: 999px; color: #fff; font-size: 10px; line-height: 1; min-width: 17px; padding: 3px 5px; text-align: center; }
				.twilio-wa-empty { color: #9ca3af; font-weight: 700; }
				.twilio-side { min-width: 0; }
				.twilio-pill { background: #eff6ff; border-radius: 999px; color: #2563eb; font-size: 12px; padding: 3px 8px; }
				.twilio-call-focus { border-top: 1px solid #eef0f3; padding-top: 12px; }
				.twilio-call-title { font-size: 16px; font-weight: 700; }
				.twilio-call-timer { font-size: 36px; font-weight: 800; margin: 12px 0; }
				.twilio-call-controls { display: flex; gap: 8px; }
				.twilio-call-assets { border-top: 1px solid #eef0f3; font-size: 12px; margin-top: 12px; padding-top: 10px; }
				.twilio-call-assets a { font-weight: 700; }
				.twilio-auto-call-dialog .modal-dialog { max-width: min(520px, calc(100vw - 32px)); }
				.twilio-auto-call-dialog .modal-body { padding: 16px; }
				.twilio-auto-call-dialog .twilio-band { margin-bottom: 0; }
				.twilio-auto-live { border-top: 1px solid #eef0f3; display: grid; gap: 8px; max-height: 220px; overflow: auto; padding-top: 10px; }
				.twilio-auto-event { border-left: 3px solid #d1d5db; padding-left: 8px; }
				.twilio-auto-event.active { border-color: #0ea5e9; }
				.twilio-auto-event.done { border-color: #16a34a; }
				.twilio-auto-event.failed { border-color: #dc2626; }
				.twilio-auto-event strong { display: block; font-size: 12px; }
				.twilio-auto-event span { color: #6b7280; display: block; font-size: 11px; overflow-wrap: anywhere; }
				.twilio-live-call { background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; margin-bottom: 12px; padding: 12px; }
				.twilio-live-call-head { align-items: center; display: flex; justify-content: space-between; margin-bottom: 10px; }
				.twilio-live-call-head strong { font-size: 13px; }
				.twilio-live-pill { background: #eef2ff; border-radius: 999px; color: #3730a3; font-size: 11px; font-weight: 800; padding: 3px 8px; }
				.twilio-live-steps { display: grid; gap: 8px; }
				.twilio-live-step { align-items: start; display: grid; gap: 9px; grid-template-columns: 22px minmax(0, 1fr); }
				.twilio-live-dot { background: #d1d5db; border-radius: 50%; height: 10px; margin: 5px auto 0; width: 10px; }
				.twilio-live-step.active .twilio-live-dot { background: #0ea5e9; box-shadow: 0 0 0 4px rgba(14, 165, 233, .12); }
				.twilio-live-step.done .twilio-live-dot { background: #16a34a; }
				.twilio-live-step.failed .twilio-live-dot { background: #dc2626; }
				.twilio-live-title { font-size: 12px; font-weight: 800; }
				.twilio-live-meta { color: #6b7280; font-size: 12px; overflow-wrap: anywhere; }
				.twilio-transcript { background: #f9fafb; border: 1px solid #eef0f3; border-radius: 6px; margin-top: 6px; max-height: 120px; overflow: auto; padding: 8px; white-space: pre-wrap; }
				.twilio-side textarea, .twilio-side select { margin-bottom: 10px; }
				.twilio-tabs { border-bottom: 1px solid #e5e7eb; display: flex; flex-wrap: wrap; gap: 20px; margin: -4px 0 16px; max-width: 100%; }
				.twilio-tabs button { background: none; border: 0; border-bottom: 2px solid transparent; font-weight: 700; padding: 8px 0; }
				.twilio-tabs button.active { border-color: #111827; }
				.twilio-context-grid { display: grid; gap: 16px; grid-template-columns: minmax(0, 1fr) 360px; min-height: 210px; }
				.twilio-guidance-list { margin: 0; padding-left: 18px; }
				.twilio-history-row { border-bottom: 1px solid #eef0f3; display: grid; gap: 8px; grid-template-columns: 130px 100px 1fr; padding: 8px 0; }
				.twilio-info-list { display: grid; gap: 14px; margin-top: 12px; }
				.twilio-info-row { align-items: center; display: grid; gap: 12px; grid-template-columns: 38px minmax(0, 1fr); }
				.twilio-info-icon { align-items: center; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 50%; display: flex; height: 38px; justify-content: center; width: 38px; }
				.twilio-detail-head { align-items: center; display: flex; gap: 12px; justify-content: space-between; margin-bottom: 14px; }
				.twilio-detail-head h3 { font-size: 16px; font-weight: 700; margin: 0; }
				.twilio-audio-list { display: grid; gap: 12px; }
				.twilio-audio-card { border: 1px solid #eef0f3; border-radius: 8px; padding: 12px; }
				.modal-dialog.modal-xl, .modal-dialog.modal-extra-large { max-width: min(1200px, calc(100vw - 32px)); }
				.twilio-workdesk-modal { overflow-x: hidden; }
				.twilio-workdesk-modal .modal-dialog { margin-left: auto; margin-right: auto; max-width: min(1180px, calc(100vw - 64px)) !important; width: auto !important; }
				.twilio-workdesk-modal .modal-content, .twilio-workdesk-modal .modal-body, .twilio-workdesk-modal .form-layout, .twilio-workdesk-modal .form-page, .twilio-workdesk-modal .form-section, .twilio-workdesk-modal .section-body { max-width: 100%; min-width: 0; overflow-x: hidden; }
				...
				.twilio-detail-dialog, .twilio-detail-dialog * { box-sizing: border-box; max-width: 100%; }
				.twilio-detail-dialog { overflow-x: hidden; width: 100%; }
				.twilio-workdesk { max-width: 100%; min-height: 520px; min-width: 0; overflow-x: hidden; }
				.twilio-workdesk-top { margin-bottom: 14px; }
				.twilio-workdesk-actions { display: flex; flex-wrap: wrap; gap: 8px; justify-content: flex-start; margin-top: 10px; }
				.twilio-workdesk-actions .btn { white-space: nowrap; }
				.twilio-workdesk-title { min-width: 0; }
				.twilio-workdesk-title h3 { font-size: 18px; font-weight: 800; margin: 0 0 4px; overflow-wrap: anywhere; }
				.twilio-call-route { align-items: center; color: #4b5563; display: flex; flex-wrap: wrap; font-size: 12px; gap: 8px; margin-top: 8px; }
				.twilio-call-route-chip { background: #f9fafb; border: 1px solid #eef0f3; border-radius: 6px; font-weight: 700; max-width: 100%; overflow-wrap: anywhere; padding: 4px 8px; }
				.twilio-call-route-icon { color: #059669; }
				.twilio-workdesk-grid { display: grid; gap: 12px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
				.twilio-workdesk-card { border: 1px solid #eef0f3; border-radius: 8px; min-width: 0; overflow-x: hidden; padding: 12px; }
				.twilio-workdesk-wide { grid-column: 1 / -1; }
				.twilio-workdesk-card h4 { font-size: 13px; font-weight: 800; margin: 0 0 10px; }
				.twilio-field-grid { display: grid; gap: 10px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
				.twilio-field { background: #f9fafb; border-radius: 6px; min-height: 54px; padding: 8px; }
				.twilio-field-label { color: #6b7280; font-size: 11px; font-weight: 700; margin-bottom: 4px; }
				.twilio-field-value { font-weight: 700; overflow-wrap: anywhere; }
				.twilio-related-row { align-items: center; border-bottom: 1px solid #eef0f3; display: grid; gap: 10px; grid-template-columns: minmax(0, 1fr) auto; padding: 8px 0; }
				.twilio-related-row:last-child { border-bottom: 0; }
				.twilio-related-title { font-weight: 700; overflow-wrap: anywhere; }
				.twilio-related-meta { color: #6b7280; font-size: 12px; overflow-wrap: anywhere; }
				.twilio-clinical-history { display: grid; gap: 12px; }
				.twilio-clinical-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px; }
				.twilio-clinical-head { align-items: center; display: flex; flex-wrap: wrap; gap: 8px; justify-content: space-between; margin-bottom: 10px; }
				.twilio-clinical-head span { color: #6b7280; font-size: 12px; }
				.twilio-clinical-section { border-top: 1px solid #f0f2f5; padding: 9px 0; }
				.twilio-clinical-label { color: #374151; font-size: 12px; font-weight: 800; margin-bottom: 4px; }
				.twilio-clinical-text { color: #4b5563; font-size: 13px; white-space: pre-wrap; word-break: break-word; }
				.twilio-clinical-med-table { margin-bottom: 0; table-layout: fixed; }
				.twilio-clinical-med-table th { color: #6b7280; font-size: 11px; font-weight: 800; }
				.twilio-clinical-med-table td { font-size: 12px; white-space: normal; word-break: break-word; }
				.twilio-empty { color: #6b7280; padding: 12px 0; }
				.twilio-wa-chat-list { background: #f9fafb; border: 1px solid #eef0f3; border-radius: 8px; display: grid; gap: 8px; margin-top: 12px; max-height: 420px; min-width: 0; overflow-x: hidden; overflow-y: auto; padding: 10px; width: 100%; }
				.twilio-wa-loader { color: #64748b; cursor: pointer; font-size: 12px; font-weight: 700; padding: 6px; text-align: center; }
				.twilio-wa-message { border: 1px solid #eef0f3; border-radius: 8px; max-width: min(82%, 860px); min-width: 0; overflow: hidden; padding: 8px 10px; }
				.twilio-wa-message.inbound { background: #fff; justify-self: start; }
				.twilio-wa-message.outbound { background: #ecfdf5; justify-self: end; }
				.twilio-wa-message-meta { color: #6b7280; font-size: 11px; font-weight: 700; margin-bottom: 4px; }
				.twilio-wa-message-body { font-size: 13px; white-space: pre-wrap; word-break: break-word; }
				.twilio-wa-media { display: block; margin-top: 6px; }
				.twilio-wa-image { border-radius: 8px; display: block; height: auto; max-height: 360px; max-width: 260px; object-fit: contain; width: auto; }
				.twilio-wa-media-link { align-items: center; background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; display: inline-flex; gap: 8px; padding: 8px 10px; text-decoration: none; }
				.twilio-wa-composer { align-items: center; background: #fff; border: 1px solid #e5e7eb; border-radius: 18px; box-shadow: 0 1px 6px rgba(15, 23, 42, .06); display: grid; gap: 6px; grid-template-columns: 32px 32px 32px minmax(0, 1fr) 42px; margin-top: 12px; max-width: 100%; min-width: 0; overflow: visible; padding: 8px 10px; width: 100%; }
				.twilio-wa-icon-btn { align-items: center; background: transparent; border: 0; color: #111827; display: inline-flex; font-size: 17px; height: 32px; justify-content: center; min-width: 32px; padding: 0; width: 32px; }
				.twilio-wa-attach-wrap, .twilio-wa-emoji-wrap { position: relative; }
				.twilio-wa-menu { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; bottom: 42px; box-shadow: 0 12px 30px rgba(15, 23, 42, .16); display: none; left: 0; min-width: 190px; padding: 8px; position: absolute; z-index: 10; }
				.twilio-wa-menu.show { display: grid; gap: 4px; }
				.twilio-wa-menu button { align-items: center; background: transparent; border: 0; border-radius: 8px; display: flex; gap: 10px; padding: 8px 10px; text-align: left; width: 100%; }
				.twilio-wa-menu button:hover { background: #f3f4f6; }
				.twilio-wa-emoji-menu { grid-template-columns: repeat(8, 30px); min-width: 276px; }
				.twilio-wa-emoji-menu button { font-size: 18px; justify-content: center; padding: 4px; }
				.twilio-wa-composer textarea { background: transparent; border: 0; box-shadow: none; height: 36px; line-height: 20px; margin: 0; max-height: 96px; min-height: 36px; min-width: 0; padding: 8px 4px; resize: none; width: 100%; }
				.twilio-wa-composer textarea:focus { border: 0; box-shadow: none; outline: 0; }
				.twilio-wa-send { align-items: center; background: #16a34a; border: 0; border-radius: 50%; color: #fff; display: inline-flex; font-size: 18px; height: 42px; justify-content: center; padding: 0; width: 42px; }
				.twilio-wa-send:disabled { opacity: .65; }
				.twilio-dialpad { align-items: center; display: grid; gap: 14px; grid-template-columns: 110px minmax(0, 1fr); }
				.twilio-pad-grid { display: grid; gap: 8px; grid-template-columns: repeat(3, 1fr); }
				.twilio-pad-grid button { background: #f3f4f6; border: 1px solid #e5e7eb; border-radius: 6px; font-weight: 700; height: 38px; }
				.twilio-wave { align-items: center; display: flex; gap: 4px; height: 120px; }
				.twilio-wave span { background: #6b7280; border-radius: 999px; display: block; width: 5px; }
				.twilio-callback-popup { align-items: flex-start; display: grid; gap: 14px; grid-template-columns: 44px minmax(0, 1fr); }
				.twilio-callback-icon { align-items: center; background: #ecfdf5; border: 1px solid #bbf7d0; border-radius: 50%; color: #16a34a; display: flex; font-size: 18px; height: 44px; justify-content: center; width: 44px; }
				.twilio-callback-popup h4 { font-size: 16px; font-weight: 800; margin: 0 0 10px; }
				.twilio-callback-row { align-items: center; border-top: 1px solid #eef0f3; display: grid; gap: 10px; grid-template-columns: 110px minmax(0, 1fr); padding: 8px 0; }
				.twilio-callback-row span { color: #6b7280; font-size: 12px; font-weight: 700; }
				.twilio-callback-row strong { overflow-wrap: anywhere; }
				.twilio-template-card { background: #f9fafb; border: 1px solid #eef0f3; border-radius: 8px; display: grid; gap: 8px; margin-top: 10px; padding: 10px; white-space: pre-wrap; word-break: break-word; }
				@media (max-width: 1100px) {
					.twilio-stats, .twilio-layout, .twilio-context-grid, .twilio-workdesk-grid, .twilio-field-grid { grid-template-columns: 1fr; }
					.twilio-console { padding: 14px; }
					.twilio-workdesk-top { display: block; }
					.twilio-workdesk-actions { justify-content: flex-start; margin-top: 10px; }
					.twilio-wa-composer { grid-template-columns: 30px 30px 30px minmax(0, 1fr) 40px; padding: 8px; }
				}
				@media (max-width: 700px) {
					.twilio-workdesk-modal .modal-dialog { max-width: calc(100vw - 20px) !important; }
					.twilio-wa-composer { border-radius: 14px; gap: 4px; grid-template-columns: 28px 28px 28px minmax(0, 1fr) 38px; padding: 7px; }
					.twilio-wa-icon-btn { font-size: 15px; height: 28px; min-width: 28px; width: 28px; }
					.twilio-wa-send { height: 38px; width: 38px; }
					.twilio-wa-menu { left: auto; right: 0; }
				}
			</style>
		`);
	}
	...
	bind() {
		...
		$main.on('click', '[data-action="open-analytics"]', () => frappe.set_route('twilio-agent-analytics'));
		...
		$(document).on('visibilitychange.twilio-agent-console', () => {
	...
	load() {
		...
		const request = frappe.call('twilio_click_to_call.api.console.get_agent_console_data', {
	...
	start_polling() {
		...
		$(window).one('beforeunload', () => {
			...
			$(document).off('visibilitychange.twilio-agent-console twilio_availability_changed.twilio-agent-console page-change.twilio-agent-console route-change.twilio-agent-console mousemove.twilio-agent-console keydown.twilio-agent-console click.twilio-agent-console scroll.twilio-agent-console touchstart.twilio-agent-console');
		});
	}
	...
	send_console_heartbeat() {
		...
		const request = frappe.call({
			method: 'twilio_click_to_call.api.console.heartbeat_agent_console',
			type: 'POST',
			freeze: false,
	...
	mark_console_offline(useKeepalive) {
		const url = '/api/method/twilio_click_to_call.api.console.mark_agent_console_offline';
		...
		frappe.call({
			method: 'twilio_click_to_call.api.console.mark_agent_console_offline',
			type: 'POST',
			freeze: false,
			args: { tab_id: this.attendance_tab_id }
		});
	...
	get_attendance_tab_id() {
		const key = 'twilio_agent_console_tab_id';
		try {
	...
	bind_activity_tracking() {
		const events = 'mousemove.twilio-agent-console keydown.twilio-agent-console click.twilio-agent-console scroll.twilio-agent-console touchstart.twilio-agent-console';
	...
	reset_idle_timer() {
		clearTimeout(this.idle_timer);
		if (!this.is_console_visible()) return;
		this.idle_timer = setTimeout(() => this.mark_idle_offline(), TWILIO_AGENT_IDLE_MS);
	}
	...
	bind_realtime() {
		...
		frappe.realtime.on('twilio_customer_callback', this.callback_handler);
		frappe.realtime.on('twilio_patient_routed_call', this.patient_routed_handler);
		frappe.realtime.on('twilio_call_disconnected', this.call_disconnected_handler);
	}

	unbind_realtime() {
		if (frappe.realtime && this.callback_handler && frappe.realtime.off) {
			frappe.realtime.off('twilio_customer_callback', this.callback_handler);
		}
		if (frappe.realtime && this.patient_routed_handler && frappe.realtime.off) {
			frappe.realtime.off('twilio_patient_routed_call', this.patient_routed_handler);
		}
		if (frappe.realtime && this.call_disconnected_handler && frappe.realtime.off) {
			frappe.realtime.off('twilio_call_disconnected', this.call_disconnected_handler);
		}
	...
	is_console_visible() {
		...
		return routeText === 'twilio-agent-console' || window.location.pathname.includes('/app/twilio-agent-console');
	}
	...
	show_customer_callback_popup(payload) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [{
				...
				fieldtype: 'HTML',
				options: `
					<div class="twilio-callback-popup">
						<div class="twilio-callback-icon"><i class="fa fa-phone"></i></div>
						<div>
							<h4>${__('Incoming customer callback')}</h4>
							<div class="twilio-callback-row"><span>${__('Customer')}</span><strong>${frappe.utils.escape_html(payload.customer_number || '-')}</strong></div>
							<div class="twilio-callback-row"><span>${__('Called DID')}</span><strong>${frappe.utils.escape_html(payload.did_number || '-')}</strong></div>
							<div class="twilio-callback-row"><span>${__('Lead')}</span><strong>${frappe.utils.escape_html(reference || payload.crm_lead || '-')}</strong></div>
						</div>
					</div>
				`
			}],
	...
	open_callback_workdesk(payload) {
		...
		frappe.call('twilio_click_to_call.api.console.get_reference_context', {
				...
				options: `
					<div class="twilio-callback-popup">
						...
						<div>
							...
							<div class="twilio-callback-row"><span>${__('Patient')}</span><strong>${frappe.utils.escape_html(payload.patient_name || payload.patient || '-')}</strong></div>
							<div class="twilio-callback-row"><span>${__('Caller')}</span><strong>${frappe.utils.escape_html(payload.customer_number || '-')}</strong></div>
							<div class="twilio-callback-row"><span>${__('Department')}</span><strong>${frappe.utils.escape_html(payload.medical_department || '-')}</strong></div>
							<div class="twilio-callback-row"><span>${__('Follow-up ID')}</span><strong>${frappe.utils.escape_html(payload.sr_followup_id || '-')}</strong></div>
						</div>
					</div>
				`
			}],
	...
	render_availability(capability, active_call) {
		...
		this.page.main.find('.twilio-state-dot').css('background', colors[status] || '#8d99a6');
	}
	...
	render_queue_meta() {
		...
		this.page.main.find('.twilio-patient-col').toggleClass('hidden', (meta.doctype || '') !== 'Patient');
		this.page.main.find('.twilio-team-col').toggleClass('hidden', (meta.doctype || '') === 'Patient');
		this.page.main.find('.twilio-lead-owner-col').toggleClass('hidden', (meta.doctype || '') === 'Patient');
	...
	row_html(row) {
		...
		return `
			<tr data-index="${row.index}" data-action="select-row">
				...
				<td><div class="twilio-person"><span class="twilio-avatar">${frappe.utils.escape_html(initials)}</span><span>${frappe.utils.escape_html(row.title || row.name || '')}</span></div></td>
				<td>${frappe.utils.escape_html(row.phone || '')}</td>
				<td>${this.missed_call_cell_html(row)}</td>
				<td>${this.whatsapp_queue_cell_html(row)}</td>
				<td class="twilio-patient-col ${this.is_patient_queue() ? '' : 'hidden'}">${frappe.utils.escape_html(row.sr_medical_department || '')}</td>
				<td class="twilio-patient-col ${this.is_patient_queue() ? '' : 'hidden'}">${frappe.utils.escape_html(row.sr_dpt_disease || '')}</td>
				<td class="twilio-patient-col ${this.is_patient_queue() ? '' : 'hidden'}">${frappe.utils.escape_html(row.sr_dpt_language || '')}</td>
				<td class="twilio-patient-col ${this.is_patient_queue() ? '' : 'hidden'}">${frappe.utils.escape_html(row.sr_followup_id || '')}</td>
				<td class="twilio-patient-col ${this.is_patient_queue() ? '' : 'hidden'}">${frappe.utils.escape_html(row.sr_followup_day || '')}</td>
				<td class="twilio-team-col ${this.is_patient_queue() ? 'hidden' : ''}">${frappe.utils.escape_html(row.team || '')}</td>
				<td class="twilio-lead-owner-col ${this.is_patient_queue() ? 'hidden' : ''}">${frappe.utils.escape_html(row.owner || '')}</td>
				<td><span class="twilio-status ${frappe.utils.escape_html(statusClass)}">${frappe.utils.escape_html(row.status || '')}</span></td>
	...
	missed_call_cell_html(row) {
		...
		const title = count
			...
			: __('No missed calls');
		const content = `
			<span class="twilio-missed-count">${frappe.utils.escape_html(String(count))}</span>
			${count ? `<span class="twilio-missed-badge"><i class="fa fa-phone"></i></span>` : ''}
		`;
		if (!count) {
			return `
				<div class="twilio-missed-cell twilio-missed-empty" title="${frappe.utils.escape_html(title)}">
					${content}
				</div>
			`;
		}
		return `
			<button type="button" class="twilio-missed-cell clickable ${isNew ? 'has-new' : ''}" data-action="open-missed-calls" title="${frappe.utils.escape_html(title)}">
				${content}
			</button>
		`;
	}
	...
	load_missed_call_seen() {
		try {
			return JSON.parse(window.localStorage.getItem(TWILIO_MISSED_CALL_SEEN_KEY) || '{}') || {};
		} catch (e) {
			return {};
		}
	}
	...
	save_missed_call_seen() {
		try {
			window.localStorage.setItem(TWILIO_MISSED_CALL_SEEN_KEY, JSON.stringify(this.state.missed_call_seen || {}));
		} catch (e) {
	...
	whatsapp_queue_cell_html(row) {
		if (!row.whatsapp_conversation) {
			return `<span class="twilio-wa-empty">-</span>`;
		}
		...
		return `
			<button class="btn btn-xs btn-default twilio-wa-queue ${className}" data-action="open-whatsapp-row" title="${frappe.utils.escape_html(title)}">
				<i class="fa fa-whatsapp"></i>
				${unread ? `<span class="twilio-wa-count">${frappe.utils.escape_html(String(unread))}</span>` : `<span>${__('Chat')}</span>`}
			</button>
		`;
	}
	...
	render_auto_live() {
		...
		const currentHtml = current.lead ? `
			<div class="twilio-auto-event active">
		...
		this.page.main.find('[data-role="auto-live"]').html(currentHtml + (events.map(event => `
			<div class="twilio-auto-event ${frappe.utils.escape_html(event.state || '')}">
	...
	show_auto_call_dialog() {
		...
		dialog.$wrapper.addClass('twilio-auto-call-dialog');
		dialog.get_close_btn().show();
	...
	auto_call_dialog_html() {
		return `
			<section class="twilio-band twilio-active">
				<div class="twilio-section-title">
					<h3>${__('Current Call')}</h3>
					<span class="twilio-pill" data-role="call-status">${__('Idle')}</span>
				</div>
				<div class="twilio-call-focus">
					<div class="twilio-call-title" data-role="focus-name">${__('No active call')}</div>
					<div class="text-muted" data-role="focus-meta">${__('Waiting for auto dial call')}</div>
					<div class="twilio-call-timer" data-role="timer">00:00</div>
					<div class="twilio-call-controls">
					...
					<div class="twilio-call-assets" data-role="call-assets"></div>
				</div>
			</section>
		`;
	}
	...
	render_call_assets(call) {
		...
		if (call.recording_url) {
			const recordingUrl = call.recording_download_url || `/api/method/twilio_click_to_call.api.recording.stream?call_log=${encodeURIComponent(call.name || '')}`;
		...
		if (call.transcript_text) {
			rows.push(`<div class="twilio-transcript">${frappe.utils.escape_html(call.transcript_text)}</div>`);
		}
	...
	render_dispositions() {
		...
		this.page.main.find('[data-role="lead-status"]').closest('.form-group, .twilio-field, .control-input-wrapper').toggle(!isPatient);
		this.page.main.find('[data-role="disposition"]').closest('.form-group, .twilio-field, .control-input-wrapper').toggle(!isPatient);
		this.page.main.find('[data-role="sr-followup-status"]').closest('.form-group, .twilio-field, .control-input-wrapper').toggle(isPatient);
		if (isPatient) {
	...
	refresh_lead_disposition_options() {
		...
		frappe.call({
			method: 'twilio_click_to_call.api.disposition.get_lead_disposition_context_api',
			args: {
	...
	render_call_summary(context) {
		...
		this.page.main.find('[data-role="tab-panel"]').html(`
			...
			<ul class="twilio-guidance-list">${script.map(line => `<li>${frappe.utils.escape_html(line)}</li>`).join('')}</ul>
		`);
	}

	info_row(icon, label, value) {
		return `
			<div class="twilio-info-row">
				<div class="twilio-info-icon"><i class="fa ${icon}"></i></div>
		...
		this.page.main.find('[data-role="tab-panel"]').html(rows.map(row => `
			<div class="twilio-audio-card">
				<div class="twilio-detail-head">
					...
					<a class="btn btn-xs btn-default" href="/app/twilio-call-log/${frappe.utils.escape_html(row.name)}">${__('Open')}</a>
				</div>
				${row.ai_summary ? `<div><strong>${__('Summary')}</strong><div>${frappe.utils.escape_html(row.ai_summary)}</div></div>` : ''}
				${row.transcript_text ? `<div class="twilio-transcript">${frappe.utils.escape_html(row.transcript_text)}</div>` : `<div class="text-muted">${frappe.utils.escape_html(row.transcript_status || __('No transcript yet'))}</div>`}
			</div>
	...
	render_insights(history) {
		...
		this.page.main.find('[data-role="tab-panel"]').html(`
			<div class="twilio-stats" style="grid-template-columns: repeat(3, minmax(0, 1fr));">
				<div class="twilio-stat"><span>${__('Previous Calls')}</span><strong>${total}</strong></div>
				<div class="twilio-stat"><span>${__('Connected')}</span><strong>${connected}</strong></div>
				<div class="twilio-stat"><span>${__('Missed')}</span><strong>${missed}</strong></div>
			</div>
		`);
	}
	...
	call_row(index) {
		...
		const request = frappe.call({
			method: 'twilio_click_to_call.api.console.get_reference_context',
			args: {
				reference_doctype: row.doctype,
				reference_name: row.name,
				lite: 1
	...
	open_missed_calls(index) {
		...
		frappe.call('twilio_click_to_call.api.console.get_reference_missed_calls', {
			reference_doctype: row.doctype,
			reference_name: row.name,
			limit: 50
		}).then((r) => {
	...
	missed_call_row_html(call) {
		...
		return `
			<div class="twilio-missed-row">
				<div class="twilio-missed-row-head">
					<strong>${frappe.utils.escape_html(call.status || __('Missed'))}</strong>
					<a class="btn btn-xs btn-default" href="/app/twilio-call-log/${frappe.utils.escape_html(call.name || '')}">${__('Open')}</a>
				</div>
				<div class="twilio-missed-row-grid">
	...
	open_detail_dialog(row, context, initial_tab) {
		...
		dialog.$wrapper.addClass('twilio-workdesk-modal');
		dialog.get_close_btn().show();
		dialog.$wrapper.on('hidden.bs.modal', () => {
		...
		const render = (tab) => {
			...
			if (tab === 'encounters') {
				...
				$body.find('[data-detail-panel]').html(this.workdesk_twilio_html(workdesk, context.history || []));
		...
		$body.html(`
			<div class="twilio-detail-dialog">
				<div class="twilio-tabs">
					...
					<button data-detail-tab="twilio">${__('Twilio Summary')}</button>
	...
	load_workdesk_tab(row, context, tab, $body, render) {
		const deferredTabs = ['encounters', 'clinical-history', 'reports', 'twilio', 'whatsapp'];
		context.loaded_workdesk_tabs = context.loaded_workdesk_tabs || { summary: true };
		...
		$body.find('[data-detail-panel]').html(`
			<div class="twilio-workdesk-card">
				<div class="twilio-empty">${__('Loading details...')}</div>
			</div>
		`);
		frappe.call({
			method: 'twilio_click_to_call.api.console.get_workdesk_tab',
			args: {
				reference_doctype: row.doctype,
				reference_name: row.name,
				tab
	...
	workdesk_lead_html(row, context) {
		...
		return `
			<div class="twilio-workdesk">
				${this.workdesk_header_html(row, context)}
				<div class="twilio-workdesk-grid">
					<div class="twilio-workdesk-card">
						<h4>${frappe.utils.escape_html(this.queue_meta_value('data_label'))}</h4>
						<div class="twilio-field-grid">
								${fields.map(field => this.workdesk_field_html(field.label, field.value, field.fieldtype, field, row)).join('') || `<div class="twilio-empty">${__('No fields found for this record.')}</div>`}
						</div>
					</div>
					...
					<div class="twilio-workdesk-card">
						...
						<ul class="twilio-guidance-list">${((context.guidance || {}).script || []).map(line => `<li>${frappe.utils.escape_html(line)}</li>`).join('')}</ul>
					</div>
					<div class="twilio-workdesk-card twilio-workdesk-wide">
						<h4>${__('Notes')}</h4>
						...
						<div class="twilio-workdesk-actions">
	...
	workdesk_lead_disposition_html(row, workdesk) {
		...
		return `
			<div class="twilio-workdesk-card">
				...
				<hr>
				<div class="twilio-related-meta">${__('Available for this lead')}</div>
				<div class="twilio-info-list">
					${options.slice(0, 8).map(row => `
						<div class="twilio-related-row">
							<div>
								<div class="twilio-related-title">${frappe.utils.escape_html(row.name || '')}</div>
								<div class="twilio-related-meta">${frappe.utils.escape_html(row.status || __('Any CRM Status'))}</div>
							</div>
						</div>
					`).join('') || `<div class="twilio-empty">${__('No active SR Lead Disposition found for this status.')}</div>`}
				</div>
			</div>
		`;
	}
	...
	workdesk_live_call_html(row) {
		...
		if (!call || !call.name) {
			return `
				<div class="twilio-live-call">
					<div class="twilio-live-call-head">
						<strong>${__('Live Call')}</strong>
						<span class="twilio-live-pill">${__('Idle')}</span>
					</div>
					<div class="twilio-live-meta">${__('Start a call to see agent and customer live events here.')}</div>
				</div>
			`;
		}
		...
		return `
			<div class="twilio-live-call">
				<div class="twilio-live-call-head">
					<strong>${__('Live Call')}</strong>
					<span class="twilio-live-pill">${frappe.utils.escape_html(call.status || __('Active'))}</span>
				</div>
				<div class="twilio-live-steps">
					${steps.map(step => `
						<div class="twilio-live-step ${frappe.utils.escape_html(step.state)}">
							<span class="twilio-live-dot"></span>
							<div>
								<div class="twilio-live-title">${frappe.utils.escape_html(step.title)}</div>
								<div class="twilio-live-meta">${frappe.utils.escape_html(step.meta)}</div>
							</div>
						</div>
					`).join('')}
				</div>
				${details ? `<div class="twilio-live-meta" style="margin-top:10px;">${frappe.utils.escape_html(details)}</div>` : ''}
			</div>
		`;
	}
	...
	workdesk_header_html(row, context) {
		...
		return `
			<div class="twilio-workdesk-top">
				<div class="twilio-workdesk-title">
					<h3>${frappe.utils.escape_html(row.title || row.name || '')}</h3>
					<div class="twilio-call-route">
						<span class="twilio-call-route-chip">${__('Agent')}: ${frappe.utils.escape_html(agent_number)}</span>
						<i class="fa fa-link twilio-call-route-icon" aria-hidden="true"></i>
						<span class="twilio-call-route-chip">${__('Customer')}: ${frappe.utils.escape_html(customer_number)}</span>
					</div>
	...
	workdesk_field_html(label, value, fieldtype, field, row) {
		...
		return `
			<div class="twilio-field">
				<div class="twilio-field-label">${frappe.utils.escape_html(label || '')}</div>
				<div class="twilio-field-value">${this.workdesk_field_value_html(value, fieldtype)}</div>
			</div>
		`;
	}
	...
	workdesk_status_field_html(field, row) {
		...
		return `
			<div class="twilio-field">
				<div class="twilio-field-label">${frappe.utils.escape_html(field.label || __('Status'))}</div>
	...
	workdesk_clinical_history_html(workdesk) {
		...
		if (!patient.name && !rows.length) {
			return `<div class="twilio-workdesk-card"><div class="twilio-empty">${__('No linked patient found for clinical history.')}</div></div>`;
		}
		...
		return `
			<div class="twilio-workdesk-card">
				...
				<hr>
				<div class="twilio-clinical-history">
					${rows.map((row) => this.workdesk_clinical_history_row_html(row)).join('') || `<div class="twilio-empty">${__('No encounters with clinical notes found.')}</div>`}
				</div>
			</div>
		`;
	}
	...
	workdesk_clinical_history_row_html(row) {
		...
		const section = (title, value) => {
			...
			return clean ? `
				<div class="twilio-clinical-section">
					<div class="twilio-clinical-label">${frappe.utils.escape_html(title)}</div>
					<div class="twilio-clinical-text">${frappe.utils.escape_html(clean)}</div>
				</div>
			` : '';
		};
		const medications = row.medications || {};
		return `
			<div class="twilio-clinical-card">
				<div class="twilio-clinical-head">
	...
	workdesk_medication_table_html(title, rows) {
		...
		return `
			<div class="twilio-clinical-section">
				<div class="twilio-clinical-label">${frappe.utils.escape_html(title)}</div>
				<div class="twilio-table-wrap">
					<table class="table table-sm twilio-clinical-med-table">
						<thead>
							<tr>
								<th>${__('Medication')}</th>
								<th>${__('Dosage')}</th>
			...
			<div class="twilio-workdesk-card">
				...
				${rows.map(row => `
					<div class="twilio-related-row">
						<div>
							<div class="twilio-related-title">${frappe.utils.escape_html(row.name || '')}</div>
							<div class="twilio-related-meta">${frappe.utils.escape_html(metaBuilder(row))}</div>
						</div>
	...
	workdesk_empty_action_html(emptyText, emptyAction) {
		if (!emptyAction) {
			return `<div class="twilio-empty">${frappe.utils.escape_html(emptyText)}</div>`;
		}
		return `
			<div class="twilio-empty">${frappe.utils.escape_html(emptyText)}</div>
			...
			<div class="twilio-workdesk-grid">
				<div class="twilio-workdesk-card">
					...
					${files.map(row => `
						<div class="twilio-related-row">
							<div>
								<div class="twilio-related-title">${frappe.utils.escape_html(row.file_name || row.name || '')}</div>
								<div class="twilio-related-meta">${frappe.utils.escape_html([row.file_type, row.attached_to_doctype, row.attached_to_name].filter(Boolean).join(' • '))}</div>
							</div>
					...
					`).join('') || `<div class="twilio-empty">${__('No reports/files found.')}</div>`}
				</div>
				<div class="twilio-workdesk-card">
					...
					${(!ocr.length && !insights.length) ? `<div class="twilio-empty">${__('No AI report summary found.')}</div>` : ''}
				</div>
			</div>
		`;
	}
	...
	workdesk_text_row(title, meta, text) {
		return `
			<div class="twilio-audio-card">
				<div class="twilio-related-title">${frappe.utils.escape_html(title || '')}</div>
				<div class="twilio-related-meta">${frappe.utils.escape_html(meta || '')}</div>
				${text ? `<div class="twilio-transcript">${frappe.utils.escape_html(String(text)).slice(0, 1200)}</div>` : ''}
			</div>
		`;
	}

	workdesk_twilio_html(workdesk, history) {
		const twilio = workdesk.twilio || {};
		const rows = this.call_history_latest_first(history || twilio.history || []);
		return `
			<div class="twilio-workdesk-grid">
				<div class="twilio-workdesk-card">
					<h4>${__('Twilio Summary')}</h4>
					<div class="twilio-field-grid">
						${this.workdesk_field_html(__('Total Calls'), twilio.total || 0)}
						${this.workdesk_field_html(__('Connected'), twilio.connected || 0)}
						${this.workdesk_field_html(__('Missed'), twilio.missed || 0)}
						${this.workdesk_field_html(__('Latest Status'), (twilio.latest || {}).status || '')}
					</div>
				</div>
				<div class="twilio-workdesk-card">
	...
	workdesk_whatsapp_html(workdesk) {
		...
		if (!wa.available) {
			return `<div class="twilio-workdesk-card"><div class="twilio-empty">${frappe.utils.escape_html(wa.message || __('WhatsApp is not available.'))}</div></div>`;
		}
		return `
			<div class="twilio-workdesk-card">
				<h4>${__('WhatsApp')}</h4>
				<div class="twilio-related-meta">${frappe.utils.escape_html(data.last_message_preview || data.ai_summary || __('WhatsApp chat preview.'))}</div>
				${wa.conversation ? this.workdesk_whatsapp_messages_html(messages, wa) : `<div class="twilio-empty">${__('No WhatsApp conversation found for this lead.')}</div>`}
				${wa.conversation ? '' : `
					<div style="margin-top:12px;">
	...
	workdesk_whatsapp_messages_html(messages, wa = {}) {
		...
		if (!messages.length) {
			return `
				<div class="twilio-wa-chat-list" data-wa-chat-list data-conversation="${frappe.utils.escape_html(wa.conversation || '')}" data-before="" data-has-more="0">
					<div class="twilio-empty">${__('No WhatsApp messages found.')}</div>
				</div>
				${this.workdesk_whatsapp_composer_html()}
			`;
		}
		return `
			<div class="twilio-wa-chat-list" data-wa-chat-list data-conversation="${frappe.utils.escape_html(wa.conversation || '')}" data-before="${frappe.utils.escape_html(before || '')}" data-has-more="${has_more}">
				${wa.has_more ? `<div class="twilio-wa-loader" data-wa-loader>${__('Scroll up to load older messages')}</div>` : ''}
	...
	workdesk_whatsapp_message_html(message) {
		...
		return `
			<div class="twilio-wa-message ${side}" data-wa-message="${frappe.utils.escape_html(message.name || '')}">
				<div class="twilio-wa-message-meta">${frappe.utils.escape_html(meta)}</div>
				${body ? `<div class="twilio-wa-message-body">${frappe.utils.escape_html(body)}</div>` : ''}
				${media}
			</div>
		`;
	}
	...
	workdesk_whatsapp_media_html(message) {
		...
		if (isImage) {
			return `
				<a class="twilio-wa-media" href="${safeUrl}" target="_blank" rel="noopener">
					<img class="twilio-wa-image" src="${safeUrl}" alt="${__('WhatsApp image')}">
				</a>
			`;
		}
		return `
			<a class="twilio-wa-media twilio-wa-media-link" href="${safeUrl}" target="_blank" rel="noopener">
	...
	workdesk_whatsapp_composer_html() {
		const emojis = ['😀', '😊', '🙏', '👍', '❤️', '😂', '🎉', '✅', '📞', '💊', '🩺', '💬', '🙌', '😇', '🤝', '⭐'];
		return `
			<div class="twilio-wa-composer">
				<div class="twilio-wa-attach-wrap">
					<button class="twilio-wa-icon-btn" type="button" data-wa-attach title="${__('Attach')}"><i class="fa fa-plus"></i></button>
					<div class="twilio-wa-menu" data-wa-attach-menu>
				...
				<button class="twilio-wa-icon-btn" type="button" data-wa-template title="${__('Template')}"><i class="fa fa-file-text-o"></i></button>
				<div class="twilio-wa-emoji-wrap">
					<button class="twilio-wa-icon-btn" type="button" data-wa-emoji title="${__('Emoji')}"><i class="fa fa-smile-o"></i></button>
					<div class="twilio-wa-menu twilio-wa-emoji-menu" data-wa-emoji-menu>
				...
				<button class="twilio-wa-send" type="button" data-wa-send title="${__('Send')}"><i class="fa fa-paper-plane"></i></button>
			</div>
		`;
	}
	...
	save_workdesk_status(row, context, $select) {
		...
		frappe.call({
			method: 'twilio_click_to_call.api.console.update_reference_status',
			type: 'POST',
			args: {
				reference_doctype: row.doctype,
				reference_name: row.name,
	...
	save_workdesk_note(row, $body) {
		...
		frappe.call({
			method: 'twilio_click_to_call.api.console.save_reference_note',
			args: {
				reference_doctype: row.doctype,
				reference_name: row.name,
				note
	...
	open_whatsapp(row, $body) {
		frappe.call('twilio_click_to_call.api.console.get_whatsapp_conversation', {
			reference_doctype: row.doctype,
			reference_name: row.name
		}).then((r) => {
	...
	refresh_inline_whatsapp($body, conversation) {
		frappe.call('twilio_click_to_call.api.console.get_whatsapp_messages', {
			conversation,
			limit: TWILIO_WHATSAPP_PAGE_SIZE
		}).then((r) => {
			...
			if ($existing.length) {
				...
				$body.find('.twilio-empty').last().replaceWith($chat);
			}
			$body.find('[data-workdesk-action="whatsapp"]').closest('div').remove();
			if (!$body.find('[data-wa-reply]').length) {
				$body.find('.twilio-workdesk-card').append(this.workdesk_whatsapp_composer_html());
			}
	...
	load_more_whatsapp_messages($list) {
		...
		frappe.call('twilio_click_to_call.api.console.get_whatsapp_messages', {
			...
			if (page.has_more) {
				$list.prepend(`<div class="twilio-wa-loader" data-wa-loader>${__('Scroll up to load older messages')}</div>`);
			}
	...
	send_workdesk_whatsapp($body) {
		...
		frappe.call({
			method: 'twilio_click_to_call.api.console.send_whatsapp_reply',
	...
	open_workdesk_attachment_dialog($body, kind) {
		...
		const input_id = is_image ? 'twilio-wa-image-upload' : 'twilio-wa-document-upload';
	...
	open_workdesk_template_dialog($body) {
		...
		frappe.call('twilio_click_to_call.api.console.get_whatsapp_templates', {
			conversation
		}).then((r) => {
	...
	show_workdesk_template_dialog($body, conversation, templates) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				{
					...
					fieldtype: 'HTML',
					options: `
						<div class="twilio-template-send">
							<label>${__('Template')}</label>
							<select class="form-control" data-wa-template-select>${options}</select>
							<div class="twilio-template-preview" data-wa-template-preview></div>
						</div>
					`
				},
				{
			...
			primary_action: (values) => {
				...
				frappe.call({
					method: 'twilio_click_to_call.api.console.send_whatsapp_template',
					args: {
						conversation,
	...
	workdesk_template_preview_html(template) {
		...
		return `
			<div class="twilio-template-card">
				${meta ? `<div class="twilio-related-meta">${frappe.utils.escape_html(meta)}</div>` : ''}
	...
	workdesk_return_key() {
		return 'twilio_agent_console:last_workdesk';
	}
	...
	detail_summary_html(row, context) {
		...
		return `
			...
			<hr>
			<ul class="twilio-guidance-list">${guidance.map(line => `<li>${frappe.utils.escape_html(line)}</li>`).join('')}</ul>
		`;
	}
	...
	detail_transcript_html(history) {
		...
		return rows.map(row => `
			<div class="twilio-audio-card">
				...
				${row.ai_summary ? `<div><strong>${__('Summary')}</strong><div>${frappe.utils.escape_html(row.ai_summary)}</div></div>` : ''}
				${row.transcript_text ? `<div class="twilio-transcript">${frappe.utils.escape_html(row.transcript_text)}</div>` : `<div class="text-muted">${frappe.utils.escape_html(row.transcript_error || row.transcript_status || __('No transcript yet'))}</div>`}
			</div>
		`).join('') || `<div class="text-muted">${__('No transcript available for this lead.')}</div>`;
	}
	...
	audio_player_html(row) {
		const url = row.recording_download_url || (row.name && row.recording_url ? `/api/method/twilio_click_to_call.api.recording.stream?call_log=${encodeURIComponent(row.name)}` : '');
		if (!url) return '';
	...
	start_call_for_row(row, patientPhone = null) {
		...
		if (row.doctype === 'Patient' && !patientPhone) {
			return frappe.call({
				method: 'twilio_click_to_call.api.call.get_patient_phone_choices',
		...
		return frappe.call({
			...
			frappe.show_alert({ message: __('Call started: {0}', [message.call_log || 'Twilio']), indicator: 'green' });
			this.load();
			return message;
		});
	}
	...
	open_auto_dial_report() {
		...
		dialog.get_field('report').$wrapper.html(`
			<div class="twilio-auto-report">
				<div class="twilio-stats" style="grid-template-columns: repeat(4, minmax(0, 1fr));">
					<div class="twilio-stat"><span>${__('Selected')}</span><strong>${total}</strong></div>
					<div class="twilio-stat"><span>${__('Started')}</span><strong>${completed}</strong></div>
					<div class="twilio-stat"><span>${__('Remaining')}</span><strong>${remaining}</strong></div>
					<div class="twilio-stat"><span>${__('Status')}</span><strong>${session.running ? __('Running') : __('Stopped')}</strong></div>
				</div>
				<div class="twilio-table-wrap">
					<table class="table table-sm twilio-table">
						<thead>
						...
						<tbody>
							${(session.results || []).map(row => `
								<tr>
									...
									<td>${frappe.utils.escape_html(row.phone || '')}</td>
									<td>${row.call_log ? `<a href="/app/twilio-call-log/${frappe.utils.escape_html(row.call_log)}">${frappe.utils.escape_html(row.call_log)}</a>` : ''}</td>
	...
	cancel_call_log(call_log, row) {
		...
		return frappe.call('twilio_click_to_call.api.call.cancel_call', { call_log }).then(() => {
			return frappe.call({
				method: 'twilio_click_to_call.api.call.get_call_status',
	...
	open_post_call_disposition_dialog(call, row, on_done, options = {}) {
		...
		if (shouldRefreshPatientOptions) {
			frappe.call('twilio_click_to_call.api.disposition.get_patient_followup_status_options_api')
				.then((r) => {
		...
		const saveDisposition = (values, isAutoSave = false) => {
			...
			frappe.call('twilio_click_to_call.api.disposition.save_disposition', {
				call_log: call.name,
			...
			fields: [
				{
					...
					options: `
						<div class="twilio-workdesk-card">
							...
							${call.ai_summary ? `<div class="twilio-related-meta">${frappe.utils.escape_html(call.ai_summary)}</div>` : ''}
						</div>
					`
				},
				{
