...
class TwilioCallLog(Document):
	def on_trash(self):
		try:
			from twilio_click_to_call.services.delete_cleanup import cleanup_call_log_reverse_links
	...
	def before_save(self):
		...
		if self.transcription_text and not self.transcript_hash:
				self.transcript_hash = hashlib.sha256(self.transcription_text.encode("utf-8")).hexdigest()
