...
INCOMING_MAPPING_JSON = (
    ...
    / "doctype"
    / "twilio_incoming_mapping"
    / "twilio_incoming_mapping.json"
)
INCOMING_AGENT_JSON = (
    APP_ROOT
    / "twilio_click_to_call"
    / "doctype"
    / "twilio_incoming_mapping_agent"
    / "twilio_incoming_mapping_agent.json"
)
...
class TestIncomingMappingSource(unittest.TestCase):
    ...
    def test_assignment_priority_is_configurable_in_twilio_settings(self):
        settings = json.loads(TWILIO_SETTINGS_JSON.read_text(encoding="utf-8"))
    ...
    def test_treat_as_new_status_uses_dedicated_dedupe_setting(self):
        ...
        field = fields.get("incoming_call_treat_as_new_statuses")
        if field:
    ...
    def test_unknown_lead_defaults_follow_standalone_provider_defaulting(self):
        source = self.inbound_source
    ...
    def test_doctype_fields_match_incoming_mapping_contract(self):
        ...
        self.assertEqual(mapping["name"], "Twilio Incoming Mapping")
        ...
        self.assertEqual(mapping_fields["agents"]["options"], "Twilio Incoming Mapping Agent")

        self.assertEqual(agent["name"], "Twilio Incoming Mapping Agent")
    ...
    def test_select_option_patch_is_registered(self):
        self.assertIn(
            "twilio_click_to_call.patches.v1_0.add_select_option_crm_lead_status",
    ...
    def test_missing_inbound_cdr_recovery_is_scheduled(self):
        ...
        self.assertIn("twilio_click_to_call.services.cdr.enqueue_missing_inbound_cdr_sync", hooks)
