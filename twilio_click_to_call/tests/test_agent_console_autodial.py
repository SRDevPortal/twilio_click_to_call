...
AGENT_CONSOLE_JS = (
    ...
    / "twilio_agent_console.js"
)
AGENT_ANALYTICS_JS = (
    Path(__file__).resolve().parents[1]
    / "twilio_click_to_call"
    / "page"
    / "twilio_agent_analytics"
    / "twilio_agent_analytics.js"
)
...
USER_MAPPING_JSON = (
    ...
    / "doctype"
    / "twilio_user_mapping"
    / "twilio_user_mapping.json"
)
AGENT_ATTENDANCE_JSON = (
    Path(__file__).resolve().parents[1]
    / "twilio_click_to_call"
    / "doctype"
    / "twilio_agent_attendance_log"
    / "twilio_agent_attendance_log.json"
)
...
class TestAgentConsoleAutoDial(unittest.TestCase):
    ...
    def test_api_terminal_statuses_include_standalone_provider_canceled_spelling(self):
        source = (
    ...
    def test_active_call_disconnect_triggers_disposition_without_page_refresh(self):
        ...
        self.assertIn("twilio_call_disconnected", realtime_source)
    ...
    def test_whatsapp_icon_colors_reflect_unread_state(self):
        self.assertIn(".twilio-wa-queue.has-new { background: #dcfce7; border-color: #86efac; color: #15803d; }", self.source)
        self.assertIn(".twilio-wa-queue.is-quiet { color: #dc2626; }", self.source)
    ...
    def test_agent_online_time_uses_persistent_attendance_log(self):
        ...
        self.assertIn("AGENT_ATTENDANCE_DOCTYPE = \"Twilio Agent Attendance Log\"", source)
        ...
        self.assertIn("Twilio Agent Attendance Log", attendance)
        self.assertIn('"agent_user"', attendance)
        self.assertIn('"online_from"', attendance)
        self.assertIn('"offline_at"', attendance)
        self.assertIn("twilio_click_to_call.api.console.close_stale_agent_attendance_sessions", hooks)
    ...
    def test_agent_desk_activity_tracks_online_time_without_call_availability(self):
        ...
        self.assertIn("/assets/twilio_click_to_call/js/availability.js", hooks)
    ...
    def test_workdesk_heavy_tabs_are_lazy_loaded(self):
        ...
        for tab in ("encounters", "clinical-history", "reports", "twilio", "whatsapp"):
    ...
    def test_workdesk_whatsapp_lazy_loads_and_sends_inline(self):
        ...
        self.assertIn("TWILIO_WHATSAPP_PAGE_SIZE = 30", self.source)
        self.assertIn("data-wa-loader", self.source)
        self.assertIn("e.key === 'Enter' && !e.shiftKey", self.source)
        self.assertIn(".twilio-detail-dialog, .twilio-detail-dialog *", self.source)
        self.assertIn(".twilio-wa-image", self.source)
        self.assertIn("limit: TWILIO_WHATSAPP_PAGE_SIZE", refresh_source)
        self.assertIn("limit: TWILIO_WHATSAPP_PAGE_SIZE", more_source)
        self.assertIn("twilio_click_to_call.api.console.send_whatsapp_reply", send_source)
        ...
        self.assertIn("twilio_click_to_call.api.console.get_whatsapp_templates", self.source)
        self.assertIn("twilio_click_to_call.api.console.send_whatsapp_template", self.source)
        self.assertIn("workdesk_whatsapp_message_body_text", self.source)
        self.assertIn("workdesk_whatsapp_media_url", self.source)
        self.assertIn("img class=\"twilio-wa-image\"", media_source)
    ...
    def test_workdesk_twilio_shows_audio_only(self):
        twilio_source = self.method_source("workdesk_twilio_html", "workdesk_whatsapp_html")
        audio_source = self.method_source("detail_audio_html", "audio_player_html")

        self.assertIn("Audio Recordings", twilio_source)
        self.assertIn("detail_audio_html(rows)", twilio_source)
        self.assertNotIn("detail_transcript_tabs_html(rows)", twilio_source)
        self.assertNotIn("__('Transcript')", twilio_source)
        self.assertNotIn("Transcript / Audio", twilio_source)
    ...
    def test_mapping_drives_lead_or_patient_queue(self):
        ...
        self.assertIn('filters["twilio_last_call_status"]', console)
    ...
    def test_agent_console_dashboard_uses_filtered_analytics(self):
        ...
        self.assertIn('"Twilio Manager"', console)
        ...
        self.assertIn("twilio-agent-analytics", analytics_json)
        self.assertIn("frappe.pages['twilio-agent-analytics']", analytics)
        ...
        self.assertIn("twilio-axis-chart", analytics)
        self.assertIn("twilio-agent-card", analytics)
        ...
        self.assertIn("twilio-spectrum", analytics)
        self.assertIn("twilio-spectrum-pulse", analytics)
        ...
        self.assertIn("frappe.set_route('twilio-agent-analytics')", self.source)
        self.assertNotIn('data-role="analytics-from-date"', self.source)

    def test_global_desk_scripts_do_not_call_twilio_apis_on_home(self):
