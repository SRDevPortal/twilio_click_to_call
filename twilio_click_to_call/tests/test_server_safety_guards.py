...
from twilio_click_to_call import hooks as app_hooks
...
class TestServerSafetyGuards(unittest.TestCase):
    ...
    def test_stale_ringing_recovery_is_bounded_and_bulk(self):
        ...
        self.assertIn("UPDATE `tabTwilio Call Log`", cdr)
        self.assertIn("UPDATE `tabTwilio User Mapping`", cdr)
    ...
    def test_guest_callbacks_are_rate_limited_and_fail_closed(self):
        ...
        self.assertIn("configured_dids", inbound)
    ...
    def test_call_log_indexes_are_declared_in_a_migration_patch(self):
        ...
        self.assertIn(
            "twilio_click_to_call.patches.v1_0.add_call_log_performance_indexes",
            patches,
        )
        for index_name in (
            "idx_twilio_call_cdr_status_creation",
            "idx_twilio_call_reference_creation",
            "idx_twilio_call_reference_activity",
            "idx_twilio_call_customer_creation",
            "idx_twilio_call_uuid",
            "idx_twilio_call_recording_id",
        ):
        ...
        self.assertIn("twilio_click_to_call.patches.v1_0.add_chat_lookup_indexes", patches)
    ...
    def test_runtime_cleanup_uses_set_based_updates(self):
        ...
        self.assertIn("update `tabTwilio Agent Attendance Log`", stale_cleanup)
        self.assertNotIn("frappe.get_all(", stale_cleanup)
        self.assertNotIn("frappe.get_all(", cleanup)
        self.assertIn("update `tabTwilio User Mapping`", cleanup)
