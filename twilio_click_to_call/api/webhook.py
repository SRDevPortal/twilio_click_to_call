...
from twilio_click_to_call.services.numbers import provider_phone_number
from twilio_click_to_call.services.settings import build_callback_url, get_inbound_callback_token, get_settings
    ...
    if dial_status:
        ...
        if str(dial_status).strip().lower().replace("_", "-") == "completed":
            if _dial_completed_without_bridge(payload, previous_status):
                ...
                doc.error_message = "Twilio Dial completed without connecting the second leg."
...
def fallback(call_log: str | None = None, token: str | None = None):
    ...
    _apply_common_payload(doc, payload)
    doc.status = "Failed"
    doc.error_message = _first_value(payload, "error", "Error", "message", "Message") or "Twilio fallback callback received."
    if not doc.end_time:
        doc.end_time = frappe.utils.now()
    doc = save_doc_latest(doc, before)
...
def transcription_event():
    ...
    if not doc:
        log_twilio_event("Transcription event ignored: matching call log not found", severity="Warning", payload=data)
        return _plain_response("IGNORED")
...
def _apply_transcription_payload(doc, data: dict, original_payload: dict | None = None) -> None:
    ...
    if error:
        ...
        doc.transcript_status = "Failed"
        doc.transcript_error = "Twilio transcription callback did not include transcript text."
...
def _find_call_log_from_provider_payload(payload: dict):
    ...
    for fieldname, value in candidates:
        if not value or not frappe.get_meta("Twilio Call Log").has_field(fieldname):
            continue
        call_log = frappe.db.get_value("Twilio Call Log", {fieldname: value}, "name", order_by="creation desc")
        if call_log:
            return frappe.get_doc("Twilio Call Log", call_log)
    return None
...
def _dial_xml(doc) -> str:
    ...
    callback_url = build_callback_url(
        "twilio_click_to_call.api.webhook.dial_callback",
        doc.name,
        doc.callback_token,
        settings,
    )
    action_url = build_callback_url(
        "twilio_click_to_call.api.webhook.dial_action",
        doc.name,
        doc.callback_token,
        settings,
    )
...
def _plain_response(text: str):
    return _raw_response(text, "text/plain", "twilio.txt")
...
def _append_callback_if_enabled(call_log: str, event: str, payload: dict) -> None:
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio callback payload append failed")
...
def _start_recording_safely(call_log: str) -> None:
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio recording start failed")
