...
from twilio_click_to_call.api.call import (
...
from twilio_click_to_call.api.recording import recording_proxy_url
from twilio_click_to_call.api.disposition import get_disposition_options_api, get_patient_followup_status_options_api
from twilio_click_to_call.services.attendance import agent_attendance_enabled
from twilio_click_to_call.services.call_status import MISSED_STATUSES, is_inbound_missed_call, status_bucket, talk_seconds
from twilio_click_to_call.services.lead_disposition import get_lead_disposition_context
from twilio_click_to_call.services.patient_routing import patient_matches_mapping
from twilio_click_to_call.services.settings import get_idle_auto_offline_config, get_settings
...
# disable_twilio_analytics_api in site_config.json.
ANALYTICS_API_ENABLED = True
AGENT_ATTENDANCE_DOCTYPE = "Twilio Agent Attendance Log"
...
def _console_session_key(user: str) -> str:
    return f"twilio_agent_console:online:{user}"


def _console_tab_key(user: str, tab_id: str) -> str:
    return f"twilio_agent_console:online:{user}:{tab_id}"


def _console_tabs_key(user: str) -> str:
    return f"twilio_agent_console:tabs:{user}"


def _console_attendance_key(user: str, shift_date: str) -> str:
    return f"twilio_agent_console:attendance:{shift_date}:{user}"


def _console_heartbeat_db_touch_key(user: str, tab_id: str) -> str:
    return f"twilio_agent_console:heartbeat_db_touch:{user}:{tab_id}"


def _console_availability_touch_key(user: str) -> str:
    return f"twilio_agent_console:availability_touch:{user}"


def _desk_activity_db_touch_key(user: str, tab_id: str) -> str:
    return f"twilio_agent_console:desk_activity_db_touch:{user}:{tab_id}"
...
def record_agent_activity(tab_id: str | None = None, route: str | None = None) -> dict[str, Any]:
    ...
    user = frappe.session.user
    if not _has_enabled_twilio_user_mapping(user):
...
def mark_agent_activity_inactive(tab_id: str | None = None) -> dict[str, Any]:
    ...
    if not still_online:
        ...
        if mapping_name:
            ...
            if current_call_log and frappe.db.exists("Twilio Call Log", current_call_log):
...
def mark_agent_console_offline(tab_id: str | None = None) -> dict[str, Any]:
    ...
    frappe.cache().delete_value(_console_session_key(user))
    _set_attendance_state(user, False, _now_ist())
    mapping_name = frappe.db.get_value("Twilio User Mapping", {"user": frappe.session.user, "enabled": 1}, "name")
    if mapping_name:
        current_call_log = frappe.db.get_value("Twilio User Mapping", mapping_name, "current_call_log")
        active = False
        if current_call_log and frappe.db.exists("Twilio Call Log", current_call_log):
            active = is_active_call_log(current_call_log)
        if not active:
...
def _mark_console_user_available(throttled: bool = False) -> None:
    ...
    current_call_log = mapping.get("current_call_log")
    if current_call_log and frappe.db.exists("Twilio Call Log", current_call_log) and is_active_call_log(current_call_log):
        return
...
def get_workdesk_tab(reference_doctype: str, reference_name: str, tab: str) -> dict[str, Any]:
    ...
    if tab == "twilio":
        history = _call_history(reference_doctype, reference_name, 8)
        return {"history": history, "twilio": _twilio_summary_from_history(history)}
    if tab == "whatsapp":
...
def get_whatsapp_conversation(reference_doctype: str, reference_name: str) -> dict[str, Any]:
    ...
    if reference_doctype == "Patient":
        try:
            ...
            frappe.log_error(frappe.get_traceback(), "Twilio Workdesk Patient WhatsApp Conversation Create Failed")
            raise
    ...
    if frappe.db.exists("DocType", "Chat Conversation"):
        try:
            ...
            frappe.log_error(frappe.get_traceback(), "Twilio Workdesk WhatsApp Conversation Create Failed")
            raise
...
def send_whatsapp_reply(conversation: str, body: str) -> dict[str, Any]:
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio Workdesk WhatsApp Send Failed")
        outbound = {
...
def get_whatsapp_templates(conversation: str, force_refresh: int | str = 0) -> dict[str, Any]:
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio Workdesk WhatsApp Templates Failed")
...
def send_whatsapp_template(
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio Workdesk WhatsApp Template Send Failed")
        outbound = {
            "conversation": conversation,
            "sent": False,
            "delivery_status": "Failed",
...
def get_analytics(
    ...
    if not ANALYTICS_API_ENABLED or frappe.conf.get("disable_twilio_analytics_api"):
        frappe.throw(_("Twilio analytics API is temporarily disabled."), frappe.PermissionError)
...
def _analytics_cache_key(arguments: dict[str, Any]) -> str:
    ...
    return f"twilio_analytics:summary:{hashlib.sha256(payload.encode()).hexdigest()}"
...
def _analytics_data(
    ...
    meta = frappe.get_meta("Twilio Call Log")
    ...
    if filters.get("lead_owners"):
        ...
        conditions.append(
            ...
            exists (
                ...
                where analytics_lead.`name` = `tabTwilio Call Log`.`reference_name`
    ...
    if filters.get("teams"):
        ...
        conditions.append(
            ...
            exists (
                ...
                where analytics_team.`name` = `tabTwilio Call Log`.`reference_name`
    ...
    if filters.get("department"):
        ...
        conditions.append(
            ...
            exists (
                ...
                where analytics_patient.`name` = `tabTwilio Call Log`.`reference_name`
...
def _attach_agent_availability(rows: list[dict[str, Any]]) -> None:
    ...
    if not users or not frappe.db.exists("DocType", "Twilio User Mapping"):
        return
...
def _has_enabled_twilio_user_mapping(user: str | None) -> bool:
    if not user or not frappe.db.exists("DocType", "Twilio User Mapping"):
        return False
    return bool(frappe.db.get_value("Twilio User Mapping", {"user": user, "enabled": 1}, "name"))
...
def _bulk_close_stale_attendance_source(source: str, stale_seconds: int, now) -> int:
    ...
    for timestamp_field in ("last_seen_at", "online_from"):
        ...
        frappe.db.sql(
            f"""
            update `tabTwilio Agent Attendance Log`
            set `status` = 'Closed',
...
def _analytics_agent_options(
    ...
    values: list[str] = []
    if frappe.db.exists("DocType", "Twilio User Mapping"):
        try:
            values.extend(
                frappe.get_all(
                    "Twilio User Mapping",
            ...
            frappe.log_error(frappe.get_traceback(), "Twilio analytics agent options failed")
...
def _active_call() -> dict[str, Any] | None:
    ...
    mapping = frappe.get_doc("Twilio User Mapping", mapping_name)
    call_log = mapping.get("current_call_log")
    if not call_log or not frappe.db.exists("Twilio Call Log", call_log):
        return {
    ...
    # callbacks/CDR jobs; polling Twilio here can block every web worker.
...
def _agent_context() -> dict[str, Any]:
    ...
    if meta.has_field("queue_source") and frappe.db.has_column("Twilio User Mapping", "queue_source"):
    ...
    for fieldname in (
        ...
        if meta.has_field(fieldname) and frappe.db.has_column("Twilio User Mapping", fieldname):
...
def _queue_order_by(meta, sort_by: str | None = None) -> str:
    ...
    options = {
        ...
        "name_asc": ("name", "asc"),
        "name_desc": ("name", "desc"),
        "next_follow_up_asc": ("twilio_next_follow_up", "asc"),
...
def _queue_filters(meta, queue_source: str | None = None, agent: dict[str, Any] | None = None, followup_day: str | None = None) -> dict[str, Any]:
    ...
    if meta.name == "CRM Lead":
        if queue_source == "Discontinued" and meta.has_field("twilio_last_call_status"):
            filters["twilio_last_call_status"] = ["in", sorted(MISSED_STATUSES | {"Cancelled", "Canceled"})]
        return filters

    if meta.has_field("disabled"):
        filters["disabled"] = 0
    if meta.has_field("twilio_do_not_call"):
        filters["twilio_do_not_call"] = 0
...
def _queue_search_rows(
    ...
    if len(digits) >= 7:
        ...
        phone_fields = (
            "sr_mobile_norm",
            "twilio_normalized_phone",
            "twilio_mobile_last10",
            "twilio_phone_last10",
            "twilio_whatsapp_last10",
            "mobile_no",
            "mobile",
            "phone",
            "phone_no",
...
def _reference_row(doctype: str, doc) -> dict[str, Any]:
    ...
    next_action = data.get("twilio_last_call_status") or data.get("twilio_next_follow_up") or data.get("encounter_date") or "Initial contact"
...
def _attach_queue_missed_calls(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not rows or not frappe.db.exists("DocType", "Twilio Call Log"):
        return rows
    ...
    call_meta = frappe.get_meta("Twilio Call Log")
...
def _reference_missed_call_rows(reference_doctype: str, reference_name: str, limit: int) -> list[dict[str, Any]]:
    if not reference_doctype or not reference_name or not frappe.db.exists("DocType", "Twilio Call Log"):
        return []
...
def _workdesk_context(
    ...
    context = {
        ...
        "patient_followup_status_options": get_patient_followup_status_options_api(),
        "patient": patient,
        "twilio": _twilio_summary_from_history(history) if history is not None else _twilio_summary(reference_doctype, reference_name),
...
def _resolve_patient(
    ...
    last10 = str(phone)[-10:]
    indexed_candidates = (
        ("twilio_mobile_last10", last10),
        ("twilio_phone_last10", last10),
        ("twilio_whatsapp_last10", last10),
        ("twilio_normalized_phone", str(phone)),
    )
...
def _twilio_summary(reference_doctype: str, reference_name: str) -> dict[str, Any]:
    history = _call_history(reference_doctype, reference_name, 8)
    return _twilio_summary_from_history(history)


def _twilio_summary_from_history(history: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    try:
        ...
        if mapped_channel:
            return {
                ...
                "pipeline_map": None,
                "routing_source": "Twilio User Mapping",
            }
