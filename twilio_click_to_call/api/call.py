...
from twilio_click_to_call.services.call_log import create_outbound_call_log, sync_linked_summaries
...
from twilio_click_to_call.services.numbers import mask_phone, normalize_phone_number, numbers_match
from twilio_click_to_call.services.patient_routing import patient_matches_mapping
from twilio_click_to_call.services.safety import assert_call_allowed, get_working_hours_block_reason
...
def get_call_capability(reference_doctype: str | None = None, reference_name: str | None = None) -> dict[str, Any]:
    ...
    if not frappe.db.exists("DocType", "Twilio Settings"):
        return {"can_call": False, "reason": _("Twilio Click To Call is not installed.")}

    settings = get_settings()
    if not settings.enabled:
        return {"can_call": False, "reason": _("Twilio Click To Call is disabled.")}

    mapping = get_user_mapping(frappe.session.user)
    if not mapping:
        return {"can_call": False, "reason": _("No active Twilio user mapping found.")}
...
def start_call(
    ...
    mapping = get_user_mapping(frappe.session.user)
    if not mapping:
        frappe.throw(_("No active Twilio user mapping found for your user."))
    unavailable_reason = get_mapping_unavailable_reason(mapping)
    if unavailable_reason:
    ...
    user_mobile = normalize_phone_number(mapping.get("agent_mobile"), default_country_code=default_country_code) or frappe.session.user
    ...
    if not caller_id:
        frappe.throw(_("Twilio caller ID is not configured."))
    ...
    call_log.request_json = json.dumps(
        {
            "transport": "twilio_voice_sdk",
            "connect_params": {"call_log": call_log.name, "token": "***"},
        },
        indent=2,
    )
...
def sync_live_call_if_finished(doc) -> None:
    ...
    try:
        response = TwilioClient(get_settings()).retrieve_live_call(call_uuid)
...
def cancel_call(call_log: str) -> dict[str, Any]:
    ...
    doc = frappe.get_doc("Twilio Call Log", call_log)
    log_twilio_event("Cancel call requested", call_log=doc.name, payload={"user": frappe.session.user, "status": doc.status})
    if "System Manager" not in frappe.get_roles() and doc.user != frappe.session.user:
        frappe.throw(_("Not permitted."))
    ...
    if call_uuid:
        try:
            response = TwilioClient(get_settings()).hangup_call(call_uuid)
            message = _("Call cancellation requested.")
            log_twilio_event("Provider hangup response received", call_log=doc.name, payload=response)
        except Exception as exc:
            error_text = str(exc)
            ...
            log_twilio_event("Provider hangup call not found; cancelled locally", call_log=doc.name, severity="Warning", payload=response)
    else:
        response = {"skipped_provider_cancel": True, "reason": "Provider call UUID was not available."}
        message = _("Queued call cleared locally.")
        log_twilio_event("Cancel skipped provider hangup; no UUID", call_log=doc.name, severity="Warning", payload=response)
...
def get_my_availability() -> dict[str, Any]:
    ...
    mapping = get_user_mapping(frappe.session.user)
    if not mapping:
        return {"is_mapped": False, "reason": _("No active Twilio user mapping found.")}
...
def set_my_availability(status: str) -> dict[str, Any]:
    ...
    if current_call_log and is_active_call_log(current_call_log):
        frappe.throw(_("You cannot change availability while a Twilio call is active."))
...
def _should_apply_idle_auto_offline(user: str | None) -> bool:
    ...
    if roles.intersection(IDLE_AUTO_OFFLINE_EXEMPT_ROLES):
        return False
    if _is_twilio_team_leader(user):
        return False
    return True


def _is_twilio_team_leader(user: str) -> bool:
    if not user:
        return False
    try:
...
def get_mapping_unavailable_reason(mapping: dict[str, Any]) -> str:
    ...
    if current_call_log and not is_active_call_log(current_call_log):
        if frappe.db.exists("Twilio Call Log", current_call_log):
    ...
    if status != "Available":
        return _("Your Twilio availability is {0}.").format(status)

    if not frappe.utils.cint(mapping.get("accept_calls")):
        return _("You are not accepting Twilio calls.")

    if current_call_log and is_active_call_log(current_call_log):
        return _("You already have an active Twilio call.")
...
def is_active_call_log(call_log: str | None) -> bool:
    ...
    row = frappe.db.get_value("Twilio Call Log", call_log, ["status", "modified"], as_dict=True)
...
def restore_mapping_after_call(call_log: str) -> None:
    ...
    user = frappe.db.get_value("Twilio Call Log", call_log, "user")
    if not user:
        return
