frappe.ui.form.on("Twilio Call Log", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("Sync CDR"), () => {
            frappe.call({
                method: "twilio_click_to_call.api.cdr.sync_call_log",
...
function recordingStreamUrl(callLog) {
    return `/api/method/twilio_click_to_call.api.recording.stream?call_log=${encodeURIComponent(callLog)}`;
}

function renderRecordingPlayer(frm) {
    frm.$wrapper.find(".twilio-recording-player").remove();
    if (!frm.doc.recording_url) return;

    const url = recordingStreamUrl(frm.doc.name);
    const html = `
        <div class="twilio-recording-player" style="margin: 12px 0; padding: 12px; border: 1px solid var(--border-color); border-radius: 6px;">
