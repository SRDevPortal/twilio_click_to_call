frappe.ui.form.on('Twilio User Mapping', {
	refresh(frm) {
		load_twilio_number_options(frm);
...
frappe.provide('twilio_click_to_call');
if (!$('#twilio-user-mapping-style').length) {
	$('head').append(`
		<style id="twilio-user-mapping-style">
			.twilio-multi-values { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 8px; }
			.twilio-route-chip { align-items: center; background: #eef6ff; border: 1px solid #d8e9ff; border-radius: 6px; color: #1f4f82; display: inline-flex; gap: 4px; padding: 3px 7px; }
			.twilio-route-chip .btn { color: #60758c; line-height: 1; margin: 0; padding: 0 0 0 2px; }
		</style>
	`);
}

function load_twilio_number_options(frm) {
	frappe.call({
...
function load_team_leader(frm) {
	...
	frappe.call({
		method: 'twilio_click_to_call.twilio_click_to_call.doctype.twilio_user_mapping.twilio_user_mapping.get_team_leader',
		args: {
			team: frm.doc.team
		}
	}).then((r) => {
...
function setup_multi_value_field(frm, opts, show) {
	...
	if (!show) {
		$wrapper.find('.twilio-multi-add-btn, .twilio-multi-values').remove();
		return;
	}

	if (!$inputArea.find('.twilio-multi-add-btn').length) {
		$inputArea.css({ display: 'flex', gap: '6px', alignItems: 'center' });
		$inputArea.children(':not(.twilio-multi-add-btn)').first().css({ flex: '1 1 auto' });
		$inputArea.append(`
			<button class="btn btn-default btn-sm twilio-multi-add-btn" type="button" title="${frappe.utils.escape_html(opts.button_title)}">
				<i class="fa fa-plus"></i>
			</button>
		`);
		$inputArea.find('.twilio-multi-add-btn').on('click', () => {
	...
	if (!$wrapper.find('.twilio-multi-values').length) {
		$wrapper.append('<div class="twilio-multi-values"></div>');
	}
	render_multi_values(frm, opts);
}
...
function render_multi_values(frm, opts) {
	...
	const $list = field.$wrapper.find('.twilio-multi-values');
	if (!$list.length) return;
	$list.html(values.length ? values.map(value => `
		<span class="twilio-route-chip" data-value="${frappe.utils.escape_html(value)}">
