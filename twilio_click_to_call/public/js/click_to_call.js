(function () {
    ...
    function ensureStyles() {
        if ($("#twilio-click-to-call-style").length) return;

        $("head").append(`
            <style id="twilio-click-to-call-style">
                .twilio-call-host {
                    position: relative;
                }
                .twilio-call-host input,
                .twilio-call-host textarea {
                    padding-right: 34px;
                }
                .twilio-call-btn {
                    align-items: center;
                ...
                .control-value .twilio-field-call {
                    margin-left: 8px;
                    position: static;
                    transform: none;
                ...
                .twilio-call-history-section {
                    border-top: 1px solid var(--border-color, #d1d8dd);
                    margin-top: 10px;
                    padding-top: 10px;
                }
                .twilio-call-history-title {
                    align-items: center;
                    display: flex;
                    font-weight: 600;
                    justify-content: space-between;
                ...
                .twilio-call-history-stats {
                    display: flex;
                    flex-wrap: wrap;
                    gap: 8px;
                    margin-bottom: 8px;
                }
                .twilio-call-history-stat {
                    background: var(--control-bg, #f7fafc);
                    border: 1px solid var(--border-color, #d1d8dd);
                    border-radius: 6px;
                    font-size: 12px;
                ...
                .twilio-call-history-row:not(:last-child) {
    ...
    function setupForm(frm) {
        ...
        frappe.call({
            method: "twilio_click_to_call.api.call.get_call_capability",
            args: {
                reference_doctype: frm.doctype,
                reference_name: frm.doc.name,
            },
        }).then((r) => {
            frm.twilio_call_capability = r.message || {};
            renderButtons(frm);
    ...
    function bindGridRender(frm) {
        if (frm.__twilio_grid_bound) return;
        frm.__twilio_grid_bound = true;
        $(frm.wrapper).on("grid-row-render.twilio-click-to-call", function () {
    ...
    function renderButtons(frm) {
        ...
        const capability = frm.twilio_call_capability || {};
    ...
    function removeButtons(frm) {
        $(frm.wrapper).find(".twilio-call-btn").remove();
        $(frm.wrapper).find(".twilio-call-host").removeClass("twilio-call-host");
    }
    ...
    function attachFieldButton(frm, fieldname) {
        ...
        $host.addClass("twilio-call-host");
        const $button = makeButton("twilio-field-call").attr("data-fieldname", fieldname);
    ...
    function renderGridButtons(frm) {
        ...
        table.grid.grid_rows.forEach((gridRow) => {
            ...
            if (!$cell.length || $cell.find(".twilio-grid-call").length) return;

            const $button = makeButton("twilio-grid-call");
            $button.on("click", (event) => {
                event.preventDefault();
                event.stopPropagation();
                confirmAndStartCall(frm, {
    ...
    function makeButton(extraClass) {
        return $(`
            <button class="twilio-call-btn ${extraClass}" type="button" title="${__("Call with Twilio")}" aria-label="${__("Call with Twilio")}">
    ...
    function renderCallHistory(frm) {
        ...
        frappe.call({
            method: "twilio_click_to_call.api.disposition.get_reference_call_summary",
            args: {
                reference_doctype: frm.doctype,
                reference_name: frm.doc.name,
            },
            ...
            $dashboard.find(".twilio-call-history-section").remove();

            const rowHtml = rows.length
                ? rows.map((row) => `
                    <div class="twilio-call-history-row">
                        <a href="/app/twilio-call-log/${encodeURIComponent(row.name)}">${escapeHtml(row.name)}</a>
                        <span>${escapeHtml(row.status || "")}</span>
                        <span>${escapeHtml(row.disposition || row.ai_disposition || "")}</span>
                        <span>${row.duration || 0}s</span>
                    </div>
                `).join("")
                : `<div class="text-muted">${__("No Twilio calls yet")}</div>`;

            $dashboard.append(`
                <div class="twilio-call-history-section">
                    <div class="twilio-call-history-title">
                        <span>${__("Twilio Calls")}</span>
                        <button class="btn btn-xs btn-default twilio-open-call-list" type="button">${__("Open Logs")}</button>
                    </div>
                    <div class="twilio-call-history-stats">
                        <span class="twilio-call-history-stat">${__("Attempts")}: ${summary.total || 0}</span>
                        <span class="twilio-call-history-stat">${__("Connected")}: ${summary.connected || 0}</span>
                        <span class="twilio-call-history-stat">${__("Missed")}: ${summary.missed || 0}</span>
                    </div>
                    ${rowHtml}
                </div>
            `);

            $dashboard.find(".twilio-open-call-list").on("click", () => {
                frappe.route_options = {
                    reference_doctype: frm.doctype,
                    reference_name: frm.doc.name,
                };
                frappe.set_route("List", "Twilio Call Log");
            });
        });
    }
    ...
    function showLivePanel(callLog, frm) {
        ...
        $(".twilio-live-panel").remove();
        const startedAt = Date.now();
        const $panel = $(`
            <div class="twilio-live-panel" data-call-log="${escapeHtml(callLog)}">
                <div class="twilio-live-panel-title">
                    <span><i class="fa fa-phone"></i> ${__("Twilio Call")}</span>
                    <span class="twilio-live-timer">00:00</span>
                </div>
                <div class="twilio-live-status">${__("Calling customer...")}</div>
                <div class="twilio-live-meta"></div>
                <div class="twilio-live-actions">
                    <button class="btn btn-xs btn-default twilio-open-log" type="button">${__("Open Log")}</button>
                    <button class="btn btn-xs btn-danger twilio-cancel-call" type="button">${__("Cancel")}</button>
                </div>
            </div>
        `);
        $("body").append($panel);

        $panel.find(".twilio-open-log").on("click", () => {
            frappe.set_route("Form", "Twilio Call Log", callLog);
        });
        $panel.find(".twilio-cancel-call").on("click", () => cancelCall(callLog));
    ...
    function pollStatus(callLog, frm, $panel, startedAt) {
        ...
        const request = frappe.call({
            ...
            updateTimer($panel, startedAt);
            $panel.find(".twilio-live-status").text(statusText(data.status));
            $panel.find(".twilio-live-meta").text([
            ...
            $panel.find(".twilio-cancel-call").toggle(Boolean(data.can_cancel));
    ...
    function cancelCall(callLog) {
        frappe.confirm(__("Cancel this Twilio call?"), () => {
            frappe.call({
                method: "twilio_click_to_call.api.call.cancel_call",
    ...
    function openDispositionDialog(callLog, status, frm) {
        ...
        const dialog = new frappe.ui.Dialog({
            ...
            primary_action(values) {
                frappe.call({
                    method: "twilio_click_to_call.api.disposition.save_disposition",
                    args: {
                        call_log: callLog,
    ...
    function loadDispositionOptions(callback) {
        frappe.call({
            method: "twilio_click_to_call.api.disposition.get_disposition_options_api",
        }).then((r) => {
    ...
    function updateTimer($panel, startedAt) {
        ...
        $panel.find(".twilio-live-timer").text(`${String(minutes).padStart(2, "0")}:${String(remaining).padStart(2, "0")}`);
    }

    function notifyCompletion(status) {
        if (!("Notification" in window) || Notification.permission !== "granted") return;
        try {
            new Notification(__("Twilio call finished"), { body: status || __("Completed") });
        } catch (e) {
    ...
    $(document).on("twilio_availability_changed", function () {
