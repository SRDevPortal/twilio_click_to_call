(function () {
    ...
    const AGENT_CONSOLE_PAGE_CACHE_VERSION_KEY = "twilio_agent_console_page_cache_version";
    try {
        if (window.localStorage) {
            ...
            if (currentVersion !== AGENT_CONSOLE_PAGE_CACHE_VERSION) {
                window.localStorage.removeItem("_page:twilio-agent-console");
    ...
    const ACTIVITY_TAB_KEY = "twilio_agent_activity_tab_id";
    const ACTIVITY_LAST_KEY = "twilio_agent_last_activity_at";
    const BREAK_STARTED_KEY = "twilio_agent_break_started_at";
    const AVAILABILITY_EVENT_KEY = "twilio_agent_availability_event";
    ...
    function refresh() {
        ...
        availabilityRequest = frappe.call({
            method: "twilio_click_to_call.api.call.get_my_availability",
        }).then((r) => {
    ...
    function ensureStyles() {
        if ($("#twilio-availability-style").length) return;

        $("head").append(`
            <style id="twilio-availability-style">
                .twilio-availability-control {
                    align-items: center;
                    display: inline-flex;
                    margin-left: 8px;
                    position: relative;
                }
                .twilio-availability-button {
                    align-items: center;
                ...
                .twilio-availability-button:hover {
                    background: #fff;
                    border-color: #c9d6e2;
                    box-shadow: 0 4px 12px rgba(15, 23, 42, .08);
                }
                .twilio-availability-dot {
                    border-radius: 50%;
                    display: inline-block;
                    height: 8px;
                    width: 8px;
                }
                .twilio-dot-green { background: #22a06b; }
                .twilio-dot-orange { background: #f59e0b; }
                .twilio-dot-yellow { background: #d6a000; }
                .twilio-dot-gray { background: #8d99a6; }
                .twilio-availability-menu {
                    background: #fff;
                    border: 0;
                    border-radius: 14px;
                ...
                .twilio-availability-control.open .twilio-availability-menu {
                    display: block;
                }
                .twilio-availability-item {
                    align-items: center;
                    appearance: none;
                    background: linear-gradient(180deg, #ffffff 0%, #f7fafc 100%);
                    border: 0 !important;
                ...
                .twilio-availability-item:focus,
                .twilio-availability-item:active {
                    border: 0 !important;
                    box-shadow: 0 1px 2px rgba(15, 23, 42, .06) !important;
                    outline: 0 !important;
                }
                .twilio-availability-item:hover {
                    background: #fff;
                    box-shadow: 0 4px 12px rgba(15, 23, 42, .08);
                }
                .twilio-availability-item.active {
                ...
                .twilio-break-online:hover {
                    background: #00866c;
                }
                .twilio-break-online:disabled {
                    cursor: not-allowed;
                    opacity: .7;
                }
            </style>
    ...
    function renderControl(data) {
        const $existing = $("#twilio-availability-control");
        ...
        $control.find(".twilio-availability-button").attr("title", data.reason || __("Twilio availability"));
        syncMenuLabels($control);
        $control.find(".twilio-availability-item").removeClass("active");
    ...
    function syncMenuLabels($control) {
        STATUSES.forEach((status) => {
            ...
            const $label = $item.find(".twilio-availability-item-label");
            if ($label.length) {
    ...
    function makeControl() {
        const $control = $(`
            <div class="twilio-availability-control" id="twilio-availability-control">
                ...
                <div class="twilio-availability-menu"></div>
            </div>
        `);
        ...
        STATUSES.forEach((status) => {
            const color = COLORS[status] || "gray";
            const $item = $(`
                <div role="button" tabindex="0" class="twilio-availability-item" data-status="${status}">
                    <span class="twilio-availability-dot twilio-dot-${color}"></span>
                    <span class="twilio-availability-item-label">${status_label(status)}</span>
                </div>
            `);
            ...
            $control.find(".twilio-availability-menu").append($item);
        });

        $control.find(".twilio-availability-button").on("click", (event) => {
            event.preventDefault();
    ...
    function setStatus(status) {
        return frappe.call({
            method: "twilio_click_to_call.api.call.set_my_availability",
            args: { status },
            freeze: true,
            ...
            currentAvailability = data;
            renderControl(data);
            $(document).trigger("twilio_availability_changed", [data]);
            broadcastAvailabilityChange(data);
            frappe.show_alert({
                message: __("Twilio availability: {0}", [status_label(data.availability_status || status)]),
    ...
    function bindBreakSyncEvents() {
        ...
        breakSyncBound = true;
        $(window).on("storage.twilio-break-sync", (event) => {
            ...
            hideBreakLock();
            refresh();
            $(document).trigger("twilio_availability_changed", [currentAvailability || {}]);
        });
        $(window).on("storage.twilio-availability-sync", (event) => {
            const original = event.originalEvent || event;
    ...
    function makeBreakLock() {
        const $existing = $("#twilio-break-lock");
        if ($existing.length) return $existing;

        const $lock = $(`
            <div class="twilio-break-lock" id="twilio-break-lock" aria-live="polite">
                <div class="twilio-break-panel">
                    <div class="twilio-break-status">
                        <span class="twilio-availability-dot twilio-dot-green"></span>
                        <span>${__("Break")}</span>
                    </div>
                    <div class="twilio-break-title">${__("You are on break")}</div>
                    <div class="twilio-break-subtitle">${__("CRM access will resume when you go online.")}</div>
                    <div class="twilio-break-timer">00:00:00</div>
                    <button type="button" class="twilio-break-online">
        ...
        `);
        $lock.find(".twilio-break-online").on("click", () => {
            const $button = $lock.find(".twilio-break-online");
    ...
    function hideBreakLock() {
        ...
        breakTimer = null;
        $("#twilio-break-lock").removeClass("active");
    }
    ...
    function startBreakTimer(startedAt) {
        ...
        const update = () => {
            ...
            $("#twilio-break-lock .twilio-break-timer").text(`${hours}:${minutes}:${seconds}`);
        };
        update();
    ...
    function recordActivity() {
        ...
        frappe.call({
            method: "twilio_click_to_call.api.console.record_agent_activity",
            type: "POST",
            freeze: false,
            args: {
    ...
    function markActivityInactive() {
        ...
        idleInactive = true;
        const url = "/api/method/twilio_click_to_call.api.console.mark_agent_activity_inactive";
        if (window.fetch) {
        ...
        frappe.call({
            method: "twilio_click_to_call.api.console.mark_agent_activity_inactive",
            type: "POST",
            freeze: false,
    ...
    function bindActivityEvents() {
        ...
        activityBound = true;
        const events = "mousemove.twilio-agent-activity keydown.twilio-agent-activity click.twilio-agent-activity scroll.twilio-agent-activity touchstart.twilio-agent-activity";
        $(document).on(events, noteActivity);
        $(document).on("visibilitychange.twilio-agent-activity", handleVisibilityChange);
        $(window).on("beforeunload.twilio-agent-activity pagehide.twilio-agent-activity", () => stopActivityTracking(false));
    }
