...
from twilio_click_to_call.services.settings import get_default_country_code, get_settings


BLOCKED_REFERENCE_FIELDS = ("twilio_do_not_call", "do_not_call", "do_not_contact")
...
def get_blocked_reason(customer_number: str, reference_doctype: str | None = None, reference_name: str | None = None) -> str:
    ...
    if not normalized or not frappe.db.exists("DocType", "Twilio Blocked Number"):
        return ""

    blocked = frappe.db.get_value(
        "Twilio Blocked Number",
    ...
    return _("This number is blocked for Twilio calls: {0}.").format(blocked.reason or blocked.name)
...
def block_number(
    *,
    ...
    existing = frappe.db.exists("Twilio Blocked Number", {"normalized_phone_number": normalized})
    if existing:
        doc = frappe.get_doc("Twilio Blocked Number", existing)
        doc.enabled = 1
...
def get_working_hours_block_reason(mapping: dict) -> str:
    ...
    if allowed_days and now_dt.strftime("%A").lower() not in allowed_days:
        return _("You are outside your configured Twilio working days.")
    ...
    return "" if inside else _("You are outside your configured Twilio working hours.")
