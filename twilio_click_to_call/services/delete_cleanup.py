...
def cleanup_issue_call_log_links(doc, method=None) -> None:
    """Break Issue <-> Twilio Call Log links before Frappe validates delete links."""
    if not doc or not doc.name or not frappe.db.exists("DocType", "Twilio Call Log"):
        return
    _clear_mappings_for_issue_call_logs(doc.name)
    if _has_field("Twilio Call Log", "issue"):
        frappe.db.sql(
            "update `tabTwilio Call Log` set `issue` = '' where `issue` = %s",
            doc.name,
        )
    if _has_field("Twilio Call Log", "reference_doctype") and _has_field("Twilio Call Log", "reference_name"):
        frappe.db.sql(
            """
...
def _clear_mappings_for_issue_call_logs(issue_name: str) -> None:
    ...
    if _has_field("Twilio Call Log", "issue"):
        frappe.db.sql(
            """
            update `tabTwilio User Mapping` mapping
            inner join `tabTwilio Call Log` call_log on call_log.`name` = mapping.`current_call_log`
...
def _clear_issue_fields_linking_call_log(call_log: str) -> None:
    ...
    link_fields = [
        ...
        if df.fieldtype == "Link" and df.options == "Twilio Call Log"
    ]
    dynamic_fields = [
    ...
    for fieldname, doctype_field in dynamic_fields:
        ...
        frappe.db.sql(
            ...
            where `{doctype_field}` = 'Twilio Call Log' and `{fieldname}` = %s
            """,
            call_log,
        )
...
def _clear_mapping_current_call(call_log: str) -> None:
    ...
    frappe.db.sql(
        "update `tabTwilio User Mapping` set `current_call_log` = '' where `current_call_log` = %s",
        call_log,
    )
