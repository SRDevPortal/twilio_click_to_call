...
def _password(settings, fieldname: str, config_key: str) -> str:
    value = ""
    try:
        value = settings.get_password(fieldname) or ""
    except Exception:
        value = ""
    return (value or frappe.conf.get(config_key) or "").strip()


def get_auth_credentials(settings=None) -> tuple[str, str]:
    """Return the Twilio Account SID and Auth Token used by the REST API."""
    settings = settings or get_settings()
    account_sid = (
        getattr(settings, "account_sid", "")
        or frappe.conf.get("twilio_account_sid")
        or ""
    ).strip()
    return account_sid, _password(settings, "auth_token", "twilio_auth_token")


def get_voice_credentials(settings=None) -> tuple[str, str, str, str]:
    """Return credentials required to mint Twilio Voice SDK access tokens."""
    settings = settings or get_settings()
    account_sid, _ = get_auth_credentials(settings)
    api_key_sid = (
        getattr(settings, "api_key_sid", "")
        or frappe.conf.get("twilio_api_key_sid")
        or ""
    ).strip()
    api_key_secret = _password(settings, "api_key_secret", "twilio_api_key_secret")
    application_sid = (
        getattr(settings, "twiml_application_sid", "")
        or frappe.conf.get("twilio_twiml_application_sid")
        or ""
    ).strip()
    return account_sid, api_key_sid, api_key_secret, application_sid
...
def get_default_country_code(settings=None) -> str:
    settings = settings or get_settings()
    return (settings.default_country_code or frappe.conf.get("twilio_default_country_code") or "+91").strip()
...
def get_caller_id(settings=None, mapping: dict | None = None) -> str:
    ...
    caller_id = (
        ...
        or frappe.conf.get("twilio_default_caller_id")
        or ""
    )
...
def get_caller_ids(settings=None) -> list[str]:
    ...
    raw_values.append(frappe.conf.get("twilio_default_caller_id") or "")
...
def get_inbound_callback_token(settings=None) -> str:
    ...
    try:
        ...
        token = ""
    return (token or frappe.conf.get("twilio_inbound_callback_token") or "").strip()
...
def get_openai_api_key(settings=None) -> str:
    ...
    return (
        api_key
        or frappe.conf.get("twilio_openai_api_key")
...
def get_disposition_options(settings=None) -> list[str]:
    try:
        from twilio_click_to_call.services.lead_disposition import get_lead_disposition_options

        options = get_lead_disposition_options()
        if options:
            return options
    except Exception:
        frappe.log_error(frappe.get_traceback(), "Twilio SR Lead Disposition options failed")
...
def get_manual_disposition_options(
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio manual SR Lead Disposition options failed")
...
def get_webhook_base_url(settings=None) -> str:
    ...
    base_url = (
        ...
        or frappe.conf.get("twilio_webhook_base_url")
...
def validate_public_callback_base_url(base_url: str) -> None:
    ...
    if parsed.scheme != "https" or not hostname:
        frappe.throw(
            _(
                "Twilio Webhook Base URL must be a public HTTPS URL. "
                "Set Twilio Settings > Webhook Base URL, for example https://dev-sr.butest.tech."
            )
        )

    if _is_internal_callback_host(hostname):
        frappe.throw(_("Twilio Webhook Base URL cannot be localhost or a private/internal host."))

    try:
        ip = ipaddress.ip_address(hostname)
        if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved:
            frappe.throw(_("Twilio Webhook Base URL must use a public internet reachable host."))
    except ValueError:
        pass
