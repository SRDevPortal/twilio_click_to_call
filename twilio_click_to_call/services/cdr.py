...
from twilio_click_to_call.services.call_log import make_outbound_call_key, sync_linked_summaries, sync_reference_links
from twilio_click_to_call.services.client import TwilioClient
...
from twilio_click_to_call.services.settings import get_settings
...
def sync_call_log_cdr(call_log: str, ignore_permissions: bool = False) -> dict:
    ...
    if not settings.enabled or not settings.enable_cdr_sync:
        frappe.throw(_("Twilio CDR Sync is disabled."))
    ...
    response = TwilioClient(settings).search_cdrs(params)
...
def enqueue_recent_cdr_sync(limit: int = 50, batch_size: int = CDR_BATCH_SIZE) -> dict:
    ...
    for start in range(0, len(names), batch_size):
        ...
        frappe.enqueue(
            "twilio_click_to_call.services.cdr.process_cdr_batch",
            queue="long",
            timeout=300,
            job_id=f"twilio-cdr-batch-{batch_hash}",
            deduplicate=True,
            call_logs=batch,
        )
...
def enqueue_missing_inbound_cdr_sync(days: int = 2) -> dict:
    ...
    for date in dates:
        frappe.enqueue(
            "twilio_click_to_call.services.cdr.sync_missing_inbound_cdrs",
            queue="long",
            timeout=300,
            job_id=f"twilio-missing-inbound-cdr-{date}",
            deduplicate=True,
            date=date,
        )
...
def sync_missing_inbound_cdrs(date: str | None = None, limit: int = 100) -> dict:
    ...
    response = TwilioClient(settings).search_cdrs({"date": date})
...
def process_cdr_batch(call_logs: list[str] | tuple[str, ...]) -> dict:
    ...
    for call_log in call_logs:
        try:
            ...
            result["failed"] += 1
            frappe.log_error(frappe.get_traceback(), "Twilio CDR sync failed")
    ...
    doc = frappe.get_doc(
        {
            ...
            "error_message": "Recovered from Twilio inbound CDR because inbound webhook did not create an ERP call log.",
        }
    )
...
def _existing_call_log_for_cdr(cdr: dict) -> str:
    ...
    for fieldname, value in (
        ...
        if not value or not meta.has_field(fieldname):
            continue
        name = frappe.db.get_value("Twilio Call Log", {fieldname: value}, "name")
        if name:
            return name
    return ""
...
def _last_outbound_for_customer(customer_number: str):
    ...
    for fieldname, value in (("normalized_customer_number", customer_number), ("customer_number", customer_number)):
        if not value:
            continue
        rows = frappe.get_all("Twilio Call Log", filters={**filters, fieldname: value}, fields=fields, order_by="creation desc", limit=1)
        if rows:
            return rows[0]
    return None
