...
from twilio_click_to_call.services.call_status import normalize_status_values
...
def enqueue_ai_disposition(call_log: str, commit: bool = True) -> None:
    ...
    try:
        frappe.enqueue(
            "twilio_click_to_call.services.ai.classify_call_log",
            queue="short",
            timeout=180,
            call_log=call_log,
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio AI disposition enqueue failed")
        frappe.db.set_value(
            "Twilio Call Log",
            call_log,
            {
...
def classify_call_log(call_log: str) -> None:
    ...
    if frappe.get_meta("Twilio Call Log").has_field("transcript_status"):
...
def classify_transcript(doc, settings) -> dict[str, Any]:
    ...
    if not api_key:
        frappe.throw("OpenAI API key is not configured for Twilio AI disposition.")
...
def on_twilio_call_log_update(doc, method: str | None = None) -> None:
    try:
        sync_provider_update_to_click_to_call_log(doc)
        restore_mapping_for_terminal_call(doc)
        maybe_enqueue_from_transcript_update(doc)
    except Exception:
        frappe.log_error(frappe.get_traceback(), "Twilio click-to-call AI disposition hook failed")
...
def normalize_provider_update_status(target_call_log: str, values: dict[str, Any]) -> dict[str, Any]:
    current_status = frappe.db.get_value("Twilio Call Log", target_call_log, "status") or ""
...
def restore_mapping_for_call_log(call_log: str) -> None:
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio click-to-call mapping restore failed")
...
def maybe_enqueue_from_transcript_update(doc) -> None:
    ...
    if meta.has_field("transcript_status") and not doc.get("transcript_status"):
        values["transcript_status"] = "Completed"
    frappe.db.set_value("Twilio Call Log", doc.name, values, update_modified=False)
...
def sync_ai_disposition_safely(doc, disposition: str) -> dict:
    try:
        ...
        if not result.get("synced"):
            log_twilio_event(
                "Twilio AI CRM Lead disposition sync skipped",
                call_log=doc.name,
                severity="Warning",
        ...
        log_twilio_event(
            "Twilio AI CRM Lead disposition sync failed",
            call_log=doc.name,
            severity="Error",
            process_type="AI Processing",
            payload={
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio AI CRM Lead disposition sync failed")
...
def add_reference_comment(call_log_doc, lead_sync: dict | None = None) -> None:
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "Twilio AI summary comment failed")
