param(
    ...
    [string]$OutputFile = "\\wsl.localhost\Ubuntu-22.04\home\mit\frappe-bench\sites\site1.local\private\files\twilio_analytics_snapshot.json",
    [int]$PageLength = 500
)
...
$headers = @{
    ...
    "User-Agent" = "Mozilla/5.0 TwilioAnalyticsLocalImport/1.0"
}
...
$snapshot = [ordered]@{}
$callLogs = Get-FrappeRows -Doctype "Twilio Call Log"
$snapshot["Twilio Call Log"] = $callLogs
$snapshot["Twilio User Mapping"] = Get-FrappeRows -Doctype "Twilio User Mapping"
